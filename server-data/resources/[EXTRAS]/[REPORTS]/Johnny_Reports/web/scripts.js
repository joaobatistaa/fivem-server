...
$(document).on('click', "#submitFeedback", function() {
	...
	$.post('https://Johnny_Reports/action', JSON.stringify({
		action: "newFeedback",
		subject: subject,
...
function viewFeedbackWindow(feedbackid) {
	...
	feedbackW.find(".player_name").html(`${data.playername}`)
	...
	if (data.category === 'player_report') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-user-alt"></i> REPORT DO JOGADOR</span>`)
	} else if (data.category === 'question') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-question"></i> QUESTÃO</span>`)
	} else {
...
function reloadFeedbackTable() {
	...
	$.each(FeedbackTable, function(id, feedback) {
		...
		if (feedback.category === 'player_report') {
			category = "<span><i class='fas fa-user-alt'></i> REPORT DO JOGADOR</span>"
		} else if (feedback.category === 'question') {
			category = "<span><i class='fas fa-question'></i> QUESTÃO</span>"
		} else {
		...
		if (feedback.concluded != true) {
			feedbackTable.append(`
				<tr class="table-row" feedbackID="${id}">
					<td>REPORT #${feedback.feedbackid}</th>
					<td>${category}</td>
					<td style="color: ${statusColor};">${concluded}</td>
					<td><button type="button" class="btn btn-view"><i class="fas fa-eye"></i> VER</button></td>
				</tr>`
			)
		}
	});
	...
	if ($(".feedbackTable .table").children().length <= 0) {
		$(".feedbackTable .feedback_subtitle").html("BOM TRABALHO! PODES AGORA RELAXAR E CONTINUAR A FAZER ROLEPLAY.");
		...
		$(".feedback_subtitle").html("HEY! PARECE QUE TENS REPORTS PARA ATENDER...");
