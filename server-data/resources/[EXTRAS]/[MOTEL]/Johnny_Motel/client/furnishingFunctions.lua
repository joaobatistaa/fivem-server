...
OpenControlMenu = function()
    local menuElements = {
        {
            ["label"] = "Colocar mobília.",
            ["action"] = "furnish"
        },
        {
            ["label"] = "Gerir mobília já colocada.",
            ["action"] = "refurnish"
        }
    }

    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "control_motel_room", {
        ["title"] = "Mobilar o Quarto",
        ["align"] = 'top-left',
...
OpenFurnishingMenu = function()
    ...
    if not cachedData["furnishings"][currentMotel] then return ESX.ShowNotification("Não tens nenhum mobília comprada.") end
    if not cachedData["furnishings"][currentMotel]["ownedFurnishing"] or #cachedData["furnishings"][currentMotel]["ownedFurnishing"] == 0 then return ESX.ShowNotification("Não tens nenhum mobília comprada.") end
...
OpenRefurnishMenu = function()
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "refurnishing_motel_room", {
        ["title"] = "Escolhe mobília para gerir.",
        ["align"] = Config.AlignMenu,
        ["elements"] = menuElements
    }, function(menuData, menuHandle)
        local action = menuData["current"]["action"]
        ...
        elseif keyLabel == "ENTER" then
            ...
            ESX.TriggerServerCallback("james_motels:saveFurnishing", function(savedFurnishing)
                if savedFurnishing then
                    ...
                    ESX.ShowNotification("Mobília colocada.")
                else
...
OpenFurnishingComputer = function()
    ...
    if not ownedMotel then return ESX.ShowNotification("Necessitas de possuir um quarto.") end
...
ComputerTick = function(currentProp)
    local PurchaseProp = function()
        ...
        ESX.TriggerServerCallback("james_motels:purchaseFurnishing", function(purchased)
            ...
            if purchased then
                ESX.ShowNotification("Compra completada com sucesso.")
            else
                ESX.ShowNotification("Erro na compra.")
            end
