    ...
    if data then
        ...
        if savedWalk then -- If someone has a better implementation which works with multichar please share it.
            ...
            if result.passed then
                Play.Notification('info', 'Estilo de andar definido (Último Guardado)!')
            end
        end
    end
    cb({})
...
RegisterCommand(cfg.commandName, function()
	if not exports['qs-housing']:inDecorate() then
...
RegisterCommand(cfg.commandNameEmote, function(_, args)
    ...
    Play.Notification('info', 'Nenhum emote com esse nome definido...')
end)
...
if cfg.defaultEmoteUseKey then
    CreateThread(function()
        while cfg.defaultEmoteKey do
            if IsControlJustPressed(0, cfg.defaultEmoteKey) and not exports['qs-housing']:inDecorate() then
                findEmote(cfg.defaultEmote)
            end
            Wait(5)
        end
