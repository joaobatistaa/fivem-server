...
local function notify(message, type)
	if type == 'info' then
		exports['Johnny_Notificacoes']:Alert("INFORMAÇÃO", "<span style='color:#c7c7c7'>"..message.."", 5000, 'info')
	elseif type == 'success' then
		exports['Johnny_Notificacoes']:Alert("SUCESSO", "<span style='color:#c7c7c7'>"..message.."", 5000, 'success')
	elseif type == 'error' then
		exports['Johnny_Notificacoes']:Alert("ERRO", "<span style='color:#c7c7c7'>"..message.."", 5000, 'error')
	else
		exports['Johnny_Notificacoes']:Alert("NOVA MENSAGEM", "<span style='color:#c7c7c7'>"..message.."", 5000, 'warning')
	end
end
...
Play.Scene = function(scene, p)
    if scene then
        ...
        if not scene.sex == 'both' and not (sex == scene.sex) then
            Play.Notification('error', 'O teu sexo não permite efetuar esta animação!')
        else
...
Play.Shared = function(shared, p)
    if shared then
        ...
        if closePed then
            ...
            Play.Notification('success', 'Pedido enviado para ' .. GetPlayerName(targetId))
