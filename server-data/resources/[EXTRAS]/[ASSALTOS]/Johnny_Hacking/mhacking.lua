	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navega com o ~y~W,A,S,D~s~ e confirma com o ~y~ESPAÇO~s~ para o bloco da esquerda.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Usa as ~y~Setas~s~ e confirma com o ~y~ENTER~s~ para o bloco da direita")
			else
