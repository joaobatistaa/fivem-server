...
AddEventHandler('okokContract:GetVehicleInfo', function(source_playername, date, description, price, source)
	...
	if closestPlayer ~= -1 and playerDistance <= 3.0 then
		...
		if DoesEntityExist(vehicle) and (vehDistance <= 3) then
			...
			exports['Johnny_Notificacoes']:Alert("ERRO", "Tens que estar perto de um <span style='color:#ff0000'>veículo</span> para fazeres uma venda!", 5000, 'error')
		end
	else
		ClearPedTasks(PlayerPedId())
		exports['Johnny_Notificacoes']:Alert("ERRO", "Não tens <span style='color:#ff0000'>jogadores</span> por perto!", 5000, 'error')
	end
end)
