...
RegisterNUICallback('zone_event', function(data, cb)
    ...
    Wait(1000)
    sent = false
    TriggerEvent('johnny_popui:closeui')
end)
...
local pop = nil
local waiting = false
RegisterNetEvent('johnny_popui:drawtextuiwithinput')
AddEventHandler('johnny_popui:drawtextuiwithinput', function(table)
...
RegisterNetEvent('johnny_popui:showui')
AddEventHandler('johnny_popui:showui', function(table)
    if not open then
        local coord = GetEntityCoords(PlayerPedId())
        Wait(1000)
        open = false
...
RegisterNetEvent('johnny_popui:closeui')
AddEventHandler('johnny_popui:closeui', function(force)
