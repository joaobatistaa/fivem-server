...
AddEventHandler('DoLongHudText', function(text, type, length, style)
	if type == 1 then
		DoCustomHudText('inform', text, length, style)
	elseif type == 2 then
		DoCustomHudText('error', text, length, style)
	elseif type == 3 then
		DoCustomHudText('success', text, length, style)
	end
