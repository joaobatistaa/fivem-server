...
AddEventHandler('d3x_vehicleshop:setVehicleOwned', function (vehicleProps, label, modeloCarro, precoCarro)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
--	local nomecarro = GetDisplayNameFromVehicleModel(vehicleProps.model)
	local matricula = vehicleProps.plate
	local dono = xPlayer.identifier
	local nome = vehicleProps.name
	...
	MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, model) VALUES (@owner, @plate, @vehicle, @model)',
	{
		...
		TriggerClientEvent('Johnny_Notificacoes:Alert', _source, "STAND", "<span style='color:#c7c7c7'>Compraste o veículo:<br> -> <span style='color:#069a19'>"..label.."</span><br>Com a matrícula:<br> -> <span style='color:#069a19'>"..vehicleProps.plate.."</span>!", 5000, 'success')
		TriggerEvent('DiscordBot:compracarros', _source, matricula, dono, label, precoCarro)
	end)
...
ESX.RegisterServerCallback('d3x_vehicleshop:buyVehicle', function (source, cb, vehicleModel)
	...
	if xPlayer.getMoney() >= vehicleData.price then
		...
		cb(true, vehicleData.price)
	elseif bankMoney >= vehicleData.price then
...
ESX.RegisterServerCallback('d3x_vehicleshop:resellVehicle', function (source, cb, plate, model, label)
	local resellPrice = 0
		...
		if result[1] then -- does the owner match?
			...
			if vehicle.model == model then
				if vehicle.plate == plate then
					...
					TriggerEvent('DiscordBot:vendacarros', source, label, plate, resellPrice, dono)
					cb(true)
				else
