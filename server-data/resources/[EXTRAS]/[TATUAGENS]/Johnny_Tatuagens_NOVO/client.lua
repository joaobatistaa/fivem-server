...
Citizen.CreateThread(function()
	AddTextEntry("ParaTattoos", "Estúdio de Tatuagens")
...
function RemoveTattoo(name, label)
	...
	exports['Johnny_Notificacoes']:Alert("SUCESSO", "<span style='color:#c7c7c7'>Removeste a tatuagem <span style='color:#069a19'><b>"..GetLabelText(label).."</b></span>!", 5000, 'success')
end

function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "Abrir Loja de Tattoo", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Mudar câmera", 21, "Mudar Opacidade", {90, 89}, "Comprar/Remover Tattoo", 191)
		scaleString = sType
	end
end

Citizen.CreateThread(function()
	JayMenu.CreateMenu("tattoo", "Estúdio de Tatuagens", function()
    ...
    while true do 
		...
		if not IsMenuOpen() then
			for _,interiorId in ipairs(Config.Shops) do
				local dstcheck = GetDistanceBetweenCoords(coords, interiorId)
				if dstcheck <= 5 then
					CanSleep = false
				...
				--print(GetInteriorFromEntity(PlayerPedId()), interiorId)
