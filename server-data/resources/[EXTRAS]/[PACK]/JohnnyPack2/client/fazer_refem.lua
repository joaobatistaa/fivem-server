...
function callTakeHostage()
	...
	if not canTakeHostage then 
		exports['mythic_notify']:DoHudText('inform', 'Precisas de uma pistola com munição para fazer alguém de refém!')
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				exports['mythic_notify']:DoHudText('inform', 'Ninguém por perto para fazer de refém!')
			end
		else
			exports['mythic_notify']:DoHudText('inform', 'Ninguém por perto para fazer de refém!')
		end
...
RegisterNetEvent('animacoes:refem')
AddEventHandler('animacoes:refem', function()
	ClearPedSecondaryTask(PlayerPedId())
	DetachEntity(PlayerPedId(), true, false)
...
Citizen.CreateThread(function()
	while true do 
		...
		if takeHostage.type == "agressor" then
			...
			--drawNativeText("Pressiona [G] para largar, [H] para matar")
