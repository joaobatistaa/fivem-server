...
local polmav_hash2 = GetHashKey("polmav3")
local goe_gioe = GetHashKey("annihilator")
local fov = (fov_max+fov_min)*0.5
local vision_state = 0 -- 0 is normal, 1 is nightmode, 2 is thermal vision
Citizen.CreateThread(function()
	while true do
        local sleep = 1000
		if IsPlayerInPolmav() or IsPlayerInPolmav2() or IsPlayerInGoeGioe() then
			sleep = 5
			...
			if IsHeliHighEnough(heli) then
				...
				if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
					...
					if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
						...
						AddTextComponentString("~r~Não podes fazer rappel, pois estás a pilotar")
...
function IsPlayerInGoeGioe()
	local lPed = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(lPed)
	return IsVehicleModel(vehicle, goe_gioe)
end
