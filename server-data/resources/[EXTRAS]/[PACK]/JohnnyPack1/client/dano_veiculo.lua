...
local tireBurstMaxNumber = cfgScript.randomTireBurstInterval * 1200; 												-- the tire burst lottery runs roughly 1200 times per minute
if cfgScript.randomTireBurstInterval ~= 0 then tireBurstLuckyNumber = math.random(tireBurstMaxNumber) end			-- If we hit this number again randomly, a tire will burst.

local fixMessagePos = math.random(repaircfgScript.fixMessageCount)
local noFixMessagePos = math.random(repaircfgScript.noFixMessageCount)

-- Display blips on map
Citizen.CreateThread(function()
	if (cfgScript.displayBlips == true) then
		for _, item in pairs(repaircfgScript.mechanics) do
...
local function isPedDrivingAVehicle()
	...
	if IsPedInAnyVehicle(ped, false) then
		...
		if GetPedInVehicleSeat(vehicle, -1) == ped then
			...
			if class ~= 15 and class ~= 16 and class ~=21 and class ~=13 and model ~= -887697393 then
				return true
			end
		end
	end
...
AddEventHandler('iens:repair', function()
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfgScript.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				SetVehicleUndriveable(vehicle,false)
				SetVehicleEngineHealth(vehicle, cfgScript.cascadingFailureThreshold + 5)
				SetVehiclePetrolTankHealth(vehicle, 750.0)
				healthEngineLast=cfgScript.cascadingFailureThreshold +5
				healthPetrolTankLast=750.0
					SetVehicleEngineOn(vehicle, true, false )
				SetVehicleOilLevel(vehicle,(GetVehicleOilLevel(vehicle)/3)-0.5)
				notification("~g~" .. repaircfgScript.fixMessages[fixMessagePos] .. ", now get to a mechanic!")
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repaircfgScript.fixMessageCount then fixMessagePos = 1 end
			else
				notification("~r~Your vehicle was too badly damaged. Unable to repair!")
			end
		else
			notification("~y~" .. repaircfgScript.noFixMessages[noFixMessagePos] )
			noFixMessagePos = noFixMessagePos + 1
			if noFixMessagePos > repaircfgScript.noFixMessageCount then noFixMessagePos = 1 end
		end
	else
...
if cfgScript.torqueMultiplierEnabled or cfgScript.preventVehicleFlip or cfgScript.limpMode then
	Citizen.CreateThread(function()
		while true do
			Citizen.Wait(0)
			if cfgScript.torqueMultiplierEnabled or cfgScript.sundayDriver or cfgScript.limpMode then
				if pedInSameVehicleLast then
					local factor = 1.0
					if cfgScript.torqueMultiplierEnabled and healthEngineNew < 900 then
						factor = (healthEngineNew+200.0) / 1100
					end
					if cfgScript.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if accelerator > 127 then
								...
								local acc = fscale(accelerator, 127.0, 254.0, 0.1, 1.0, 10.0-(cfgScript.sundayDriverAcceleratorCurve*2.0))
								factor = factor * acc
							end
							if brake > 127 then
								-- Forward and braking
								isBrakingForward = true
								brk = fscale(brake, 127.0, 254.0, 0.01, fBrakeForce, 10.0-(cfgScript.sundayDriverBrakeCurve*2.0))
							end
						elseif speed <= -1.0 then
							-- Going reverse
							if brake > 127 then
								-- Reversing and accelerating (using the brake)
								local rev = fscale(brake, 127.0, 254.0, 0.1, 1.0, 10.0-(cfgScript.sundayDriverAcceleratorCurve*2.0))
								factor = factor * rev
							end
							if accelerator > 127 then
								-- Reversing and braking (Using the accelerator)
								isBrakingReverse = true
								brk = fscale(accelerator, 127.0, 254.0, 0.01, fBrakeForce, 10.0-(cfgScript.sundayDriverBrakeCurve*2.0))
							end
						else
					...
					if cfgScript.limpMode == true and healthEngineNew < cfgScript.engineSafeGuard + 5 then
						factor = cfgScript.limpModeMultiplier
					end
			...
			if cfgScript.preventVehicleFlip then
...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			healthEngineDeltaScaled = healthEngineDelta * cfgScript.damageFactorEngine * cfgScript.classDamageMultiplier[vehicleClass]

			healthBodyCurrent = GetVehicleBodyHealth(vehicle)
			if healthBodyCurrent == 1000 then healthBodyLast = 1000.0 end
			healthBodyNew = healthBodyCurrent
			healthBodyDelta = healthBodyLast - healthBodyCurrent
			healthBodyDeltaScaled = healthBodyDelta * cfgScript.damageFactorBody * cfgScript.classDamageMultiplier[vehicleClass]

			healthPetrolTankCurrent = GetVehiclePetrolTankHealth(vehicle)
			if cfgScript.compatibilityMode and healthPetrolTankCurrent < 1 then
			...
			healthPetrolTankDeltaScaled = healthPetrolTankDelta * cfgScript.damageFactorPetrolTank * cfgScript.classDamageMultiplier[vehicleClass]

			if healthEngineCurrent > cfgScript.engineSafeGuard+1 then
				SetVehicleUndriveable(vehicle,false)
			end

			if healthEngineCurrent <= cfgScript.engineSafeGuard+1 and cfgScript.limpMode == false then
			...
			if pedInSameVehicleLast == true then
				...
				if healthEngineCurrent ~= 1000.0 or healthBodyCurrent ~= 1000.0 or healthPetrolTankCurrent ~= 1000.0 then
					...
					if healthEngineCombinedDelta > (healthEngineCurrent - cfgScript.engineSafeGuard) then
					...
					if healthEngineCombinedDelta > healthEngineCurrent then
						healthEngineCombinedDelta = healthEngineCurrent - (cfgScript.cascadingFailureThreshold / 5)
					end
					...
					if healthEngineNew > (cfgScript.cascadingFailureThreshold + 5) and healthEngineNew < cfgScript.degradingFailureThreshold then
						healthEngineNew = healthEngineNew-(0.038 * cfgScript.degradingHealthSpeedFactor)
					end

					-- If Damage is near catastrophic, cascade the failure
					if healthEngineNew < cfgScript.cascadingFailureThreshold then
						healthEngineNew = healthEngineNew-(0.1 * cfgScript.cascadingFailureSpeedFactor)
					end

					-- Prevent Engine going to or below zero. Ensures you can reenter a damaged car.
					if healthEngineNew < cfgScript.engineSafeGuard then
						healthEngineNew = cfgScript.engineSafeGuard
					end

					-- Prevent Explosions
					if cfgScript.compatibilityMode == false and healthPetrolTankCurrent < 750 then
				...
				local newFDeformationDamageMult = fDeformationDamageMult ^ cfgScript.deformationExponent	-- Pull the handling file value closer to 1
				if cfgScript.deformationMultiplier ~= -1 then SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fDeformationDamageMult', newFDeformationDamageMult * cfgScript.deformationMultiplier) end  -- Multiply by our factor
				if cfgScript.weaponsDamageMultiplier ~= -1 then SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fWeaponDamageMult', cfgScript.weaponsDamageMultiplier/cfgScript.damageFactorBody) end -- Set weaponsDamageMultiplier and compensate for damageFactorBody
				...
				local newFCollisionDamageMultiplier = fCollisionDamageMult ^ cfgScript.collisionDamageExponent	-- Pull the handling file value closer to 1
				...
				--Modify it by pulling all number a towards 1.0
				local newFEngineDamageMult = fEngineDamageMult ^ cfgScript.engineDamageExponent	-- Pull the handling file value closer to 1
				...
				if healthBodyCurrent < cfgScript.cascadingFailureThreshold then
					healthBodyNew = cfgScript.cascadingFailureThreshold
				end
			...
			lastVehicle=vehicle
			if cfgScript.randomTireBurstInterval ~= 0 and GetEntitySpeed(vehicle) > 10 then tireBurstLottery() end
		else
			if pedInSameVehicleLast == true then
				-- We just got out of the vehicle
				lastVehicle = GetVehiclePedIsIn(ped, true)				
				if cfgScript.deformationMultiplier ~= -1 then SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fDeformationDamageMult', fDeformationDamageMult) end -- Restore deformation multiplier
				SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fBrakeForce', fBrakeForce)  -- Restore Brake Force multiplier
				if cfgScript.weaponsDamageMultiplier ~= -1 then SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fWeaponDamageMult', cfgScript.weaponsDamageMultiplier) end	-- Since we are out of the vehicle, we should no longer compensate for bodyDamageFactor
