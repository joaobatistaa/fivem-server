...
Citizen.CreateThread(function()
	TriggerEvent('chat:addSuggestion', '/servicocomunitario', 'Colocar um jogador no serviço comunitário.', {
		{ name="id do jogador", help="id do jogador"},
		{ name="quantidade de ações", help="quantidade de ações [sugerido: 10-40]"}
	})

	TriggerEvent('chat:addSuggestion', '/retirarservicocomunitario', 'Retirar um jogador do serviço comunitário.', {
		{ name="id do jogador", help="id do jogador"}
	})
end)
...
AddEventHandler('esx_communityservice:inCommunityService', function(actions_remaining)
	...
	while actionsRemaining > 0 and communityServiceFinished ~= true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.ServiceLocation.x, Config.ServiceLocation.y, Config.ServiceLocation.z) > 45 then
			ESX.Game.Teleport(playerPed, Config.ServiceLocation)
			exports['Johnny_Notificacoes']:Alert("ERRO", "<span style='color:#c7c7c7'>Não podes escapar! O tempo do teu <span style='color:#ff0000'>serviço comunitário</span> foi aumentado!", 5000, 'error')
...
Citizen.CreateThread(function()
	while true do
		...
		if actionsRemaining > 0 and isSentenced then
			draw2dText('Precisas de fazer mais ~b~'..ESX.Math.Round(actionsRemaining)..'~s~ ações para terminar o serviço comunitário!', { 0.175, 0.955 })
			...
			for i = 1, #availableActions do
				...
				if distance < 1.5 then
					DisplayHelpText('Pressiona ~INPUT_CONTEXT~ para começar.')
