	...
	while true do
		...
		if IsPedSittingInAnyVehicle(PlayerPedId()) then 
			for i = 1, #vehicleWashStation do
				...
				if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garageCoords2[1], garageCoords2[2], garageCoords2[3], true ) < 25 then
					letSleep = false
				...
				if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garageCoords2[1], garageCoords2[2], garageCoords2[3], true ) < 5 then
					if mycie == false then
					HelpPromt('Pressiona ~INPUT_CONTEXT~ para ~b~lavar ~s~o veículo por ~g~(1000€)')
					if IsControlJustPressed(1, Key) then
						TriggerServerEvent('es_better_carwash:checkmoney')
					end
				else
					HelpPromt('Pressiona ~INPUT_CONTEXT~ para ~o~parar')
					if IsControlJustPressed(1, Key) then
						mycie = false
						timer2 = false
    ...
    Citizen.CreateThread(function()
		while timer2 do
			...
			elseif (timer == 0) then
				...
				exports['mythic_notify']:DoHudText('success', 'O teu veículo foi limpo por '..price..'€')
        ...
        while true do
			...
			if mycie == true then
				...
				if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garageCoords3[1], garageCoords3[2], garageCoords3[3], true ) < 4 then
					DrawText3D(garageCoords3[1], garageCoords3[2], garageCoords3[3] + 3, '~b~Lavagem em progrsso... ~s~Tempo Restante:~b~ '.. timer ..' ~s~segundos.')
				end
				end
			end
		end
...
AddEventHandler('es_better_carwash:notenoughmoney', function ()
	exports['mythic_notify']:DoHudText('error', 'Não tens dinheiro suficiente!')
