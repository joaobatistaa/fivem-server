if Config.DisplayPage["BATTLEPASSPAGE"] then
    ...
    RegisterCommand('givebattlepass', function(source, args)
        ...
        if tonumber(args[1]) and (identifier == 'steam:110000115708986' or identifier == 'steam:1100001470baaf7') then
            ...
            if identifier then
				...
				TriggerClientEvent('chat:addMessage', -1, {
					template = '<div class="chat-message advertisement"><i class="fas fa-medal"></i> <b><span style="color: #81db44">{0}</span>&nbsp;|&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
					args = { "PASSE DE BATALHA", "O Jogador "..playerName.." adquiriu o Passe de Batalha!", time }
				})
            end
        else
