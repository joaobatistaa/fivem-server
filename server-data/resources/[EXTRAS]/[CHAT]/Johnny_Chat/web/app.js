window.APP = {
	...
	mounted() {
		post('http://Johnny_Chat/loaded', JSON.stringify({}));
	...
	methods: {
		...
		send(e) {
			if(this.message !== '') {
				post('http://Johnny_Chat/chatResult', JSON.stringify({
		...
		hideInput(canceled = false) {
			if (canceled) {
				post('http://Johnny_Chat/chatResult', JSON.stringify({ canceled }));
			}
			this.message = '';
