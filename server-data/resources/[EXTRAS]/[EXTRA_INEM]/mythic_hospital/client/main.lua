local hospitalCheckin = { x = 308.64, y = -591.99, z = 43.3, h = 199.2 }
...
AddEventHandler('mythic_hospital:client:RPSendToBed', function(id, data)
    ...
    Citizen.CreateThread(function()
        while bedOccupyingData ~= nil do
            Citizen.Wait(1)
            PrintHelpText('Pressiona ~INPUT_VEH_DUCK~ para levantares')
...
AddEventHandler('mythic_hospital:client:SendToBed', function(id, data)
    ...
    Citizen.CreateThread(function ()
        ...
        exports['mythic_notify']:DoHudText('inform', 'Os médicos estão a tratar-te')
...
AddEventHandler('mythic_hospital:client:FinishServices', function()
    ...
    exports['mythic_notify']:DoHudText('inform', 'Foste tratado e pagaste a fatura')
    LeaveBed()
end)
        ...
        if distance < 10 then
            ...
            if not IsPedInAnyVehicle(PlayerPedId(), true) then
                if distance < 3 then
                    ...
                    ESX.Game.Utils.DrawText3D(vector3(hospitalCheckin.x, hospitalCheckin.y, hospitalCheckin.z + 0.3), '[E] - Check in', 0.4)
					if IsControlJustReleased(0, 54) then
                        if (GetEntityHealth(PlayerPedId()) < 200) or (IsInjuredOrBleeding()) then
                            exports['progressbar']:Progress({
                                name = "hospital_action",
                                duration = 10500,
                                label = "A fazer check-in...",
                                useWhileDead = true,
                                canCancel = true,
                                controlDisables = {
                            ...
                            exports['mythic_notify']:DoHudText('error', 'Não precisas de atenção médica')
                        end
                    end
                end
            end
