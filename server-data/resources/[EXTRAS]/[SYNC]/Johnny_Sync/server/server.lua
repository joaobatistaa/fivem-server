...
local function setWeather(weather)
	...
	self.weather = string.upper(weather)
	local data = {}
...
local function setTime(hour, minute)
	local data = {}
	data.hours = tonumber(hour)
	data.mins = tonumber(minute) or 0

    if argh == nil or argh < 0 or argh > 24 or argm < 0 or argm > 60 then
        return false
    end
...
local function setTimeFreeze(state)
    if state == nil then state = not self.freeze end
    if state then self.freeze = true
    else self.freeze = false end
	local data = {}
