...
Citizen.CreateThread(function()
	...
	while true do 
		...
		elseif not nui then
			...
			if distance < 2 and vehicle ~= 0 then
				drawTxt("Pressiona [E] para abrir o menu de tunagem",4,0.5,0.93,0.50,255,255,255,180)
...
RegisterNetEvent("menutunagem:admin")
AddEventHandler("menutunagem:admin", function(a,b)
	modoadmin = true
	vehicle = getNearestVehicle(7)
	while vehicle == nil do
		vehicle = getNearestVehicle(7)
...
AddEventHandler("Notify", function(a,b)
	...
	if a == 'sucesso' then
		exports['Johnny_Notificacoes']:Alert("SUCESSO", "<span style='color:#c7c7c7'>"..b.."</span>", 5000, 'success')
	else
		exports['Johnny_Notificacoes']:Alert("ERRO", "<span style='color:#c7c7c7'>"..b.."</span>", 5000, 'error')
	end
end)
...
function isVehicleTooFar(veh)
	Citizen.CreateThread(function()
		while nui do
			...
			if distance > 7 then
				NOOBKING.closeNui()
				TriggerEvent("Notify","aviso","Afastaste-te muito do veículo!",7000)
			end
			Citizen.Wait(500)
		end
	end)
...
function getNearestBennys()
	...
	for i,j in ipairs(locais) do
		local distance = #(playercoords - locais[i].coords)
		if distance < 3 then
			if locais[i].job == PlayerData.job.name then
				return locais[i]
			end
		end
	end
