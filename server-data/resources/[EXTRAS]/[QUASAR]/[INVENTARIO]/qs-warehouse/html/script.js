...
Padlock.Close = function() {
    ...
    $.post('http://qs-warehouse/PadLockClose');
}
...
Keypad.Close = function(data) {
    $("#keypad").fadeOut(300);
    $.post('http://qs-warehouse/CombinationFail');
}
...
function submitForm(e) {
    $("#keypad").fadeOut(300);
    $.post("http://qs-warehouse/TryCombination", JSON.stringify({
        combination: e.value,
    }));
};
...
findCombo = function(comboArr){
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        ...
        if (angle > (comboArr[i] - numOffset) * tickAngle &&
            ...
            // on unlock
            $.post('http://qs-warehouse/callcops');
            if (i == comboArr.length - 1) {
                // unlock :)
                $.post('http://qs-warehouse/PadLockSuccess');
                Padlock.Close();
            }
        }
        break;
...
$(function () {
    ...
    document.onkeyup = function (data) {
        if (data.which == 27 ) {
            ...
            $.post('http://qs-warehouse/exitPadlock');
        }
    };
}); //docready
...
function unlock() {
    gameOver = true;
    $.post('http://qs-warehouse/success');
    solveDeg = (Math.random() * 180) - 90
    solvePadding = 4
    maxDistFromSolve = 45
    pinRot = 1
