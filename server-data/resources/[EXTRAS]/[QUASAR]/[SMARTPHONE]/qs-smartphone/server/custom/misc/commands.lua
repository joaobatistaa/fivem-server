RegisterCommand('enviaremail', function (source, args)
	local src = source
...
RegisterCommand('darverificado', function(source, args)
...
RegisterCommand('retirarverificado', function(source, args)
    local player = ESX.GetPlayerFromId(source)
    if player.getGroup() == 'superadmin' then
		local target = GetPlayerFromIdFramework(tonumber(args[1]))
		if target then
