...
MySQL.ready(function()
	...
	for i=1, #result, 1 do
		empresas[result[i].name]        = result[i]
		empresas[result[i].name].grades = {}
	end
end)
            ...
            if invoices[1] ~= nil then
                for k, v in pairs(invoices) do
                    if v.status == 'unpaid' then
						local empresa = false
						if string.find(v.author_identifier, "society_") then
							empresa = true
						else
                        ...
                        v.number = ''
						if empresa then
							v.number = empresas[v.author_identifier].label --ESX.GetAccount(v.author_identifier).label
						else
							if Ply ~= nil then
								v.number = QS.GetPlayerFromId(source).PlayerData.charinfo.phone
							else
