...
$('.btn-back').click(function(){
  ...
  $('#btn-upload').click(function() {
    $('#home').hide();
    $('#upload').show();
    $.post('http://WTRP_DNA/upload', JSON.stringify({}));
  });
      ...
      if ($('#remove-id').val() != '') {
        if (parseInt($('#remove-id').val())) {
          if ($('#remove-id').val().includes("#m")) {
            $.post('http://WTRP_DNA/remove', JSON.stringify({match: $('#remove-id').val()}));
          } else {
            $.post('http://WTRP_DNA/remove', JSON.stringify({id: $('#remove-id').val()}));
          }
        } else {
          $.post('http://WTRP_DNA/remove', JSON.stringify({name: $('#remove-id').val()}));
        }
      }
    } else {
      $('#remove-message').css('background', '#ae0e00');
  ...
  $('#btn-match-go').click(function() {
    if ($('#match-input').val() != '') {
      $.post('http://WTRP_DNA/match', JSON.stringify({id: $('#match-input').val()}));
    }
  });

  $('#btn-lab').click(function() {
    $('#home').hide();
    $.post('http://WTRP_DNA/fetch', JSON.stringify({type: "match"}));
    $('#lab').show();
  });

  $('#btn-brottsplatser').click(function() {
    $('#home').hide();
    $.post('http://WTRP_DNA/fetch', JSON.stringify({type: "murder"}));
  ...
  $('#btn-personer').click(function() {
    $('#home').hide();
    $.post('http://WTRP_DNA/fetch', JSON.stringify({type: "prov"}));
  ...
  $(document).keyup(function(e) {
     if (e.keyCode == 27) {
       $('#wrapper').hide();
       resetAll()
       $.post('http://WTRP_DNA/escape', JSON.stringify({}));
    }
  });
});
