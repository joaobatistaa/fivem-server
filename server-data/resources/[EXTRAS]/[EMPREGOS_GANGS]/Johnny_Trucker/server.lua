		...
		for k,v in pairs(data) do
			...
			if Config.trabalhos.gera_dinheiro_offline or source then
				if tryGetTruckerMoney(v.user_id,v.price + v.price_per_km) then
					...
					MySQL.Sync.execute(sql, {['@driver_id'] = v.driver_id});
					if source then
						TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['driver_failed']:format(v.name))
					end
				end
		...
		for k,v in pairs(data) do
			if v.timer + Config.emprestimos.cooldown < os.time() then
				...
				if tryGetTruckerMoney(v.user_id,v.day_cost) then
					...
					if source then
						TriggerClientEvent("Johnny_Trucker:Notify",source,"importante",Lang[Config.lang]['no_loan_money'])
					else
									...
									if Config.habilidade_distancia[tonumber(query_users[1].distance)] >= tonumber(distance) then
										if tonumber(query[1].contract_type) == 0 then
											...
											if query_truck and query_truck[1] then
												...
												TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['own_truck'])
											end
										end
									else
										TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['no_skill_1'])
									end
								else
									TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['no_skill_2'])
								end
							else
								TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['no_skill_3'])
							end
						else
							TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['no_skill_4'])
						end
					else
						TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['no_skill_5'])
					end
				end
			else
				TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['job_already_started'])
			end
		end
...
AddEventHandler("truck_logistics:upgradeSkill",function(data)
	...
	if user_id then
		...
		if query.skill_points >= (data.value - query[data.id]) then
			...
			TriggerClientEvent("Johnny_Trucker:Notify",source,"sucesso",Lang[Config.lang]['upgraded_skill'])
			openUI(source,true)
		else
			TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['insufficient_skill_points'])
		end
	end
end)
		...
		if query then
			...
			if amount > 0 then
				if tryGetTruckerMoney(user_id,amount) then
					...
					TriggerClientEvent("Johnny_Trucker:Notify",source,"sucesso",Lang[Config.lang]['repaired'])
					openUI(source,true)
				else
					TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['insufficiente_funds'])
				end
			else
				TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['not_repaired'])
			end
		else
			TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['have_no_truck'])
		end
	end
end)
...
AddEventHandler("truck_logistics:buyTruck",function(data)
	...
	if user_id then
		if tryGetTruckerMoney(user_id,tonumber(data.price)) then
			...
			TriggerClientEvent("Johnny_Trucker:Notify",source,"sucesso",Lang[Config.lang]['bought'])
	...
	if debug_cooldown[source] == nil then
		...
		if user_id then
			...
			if query then 
				...
				TriggerClientEvent("Johnny_Trucker:Notify",source,"sucesso",Lang[Config.lang]['sold'])
	...
	if user_id then
		...
		if count <= Config.motoristas.max_motoristas_por_player then
			...
			if tryGetTruckerMoney(user_id,query.price) then
				...
				TriggerClientEvent("Johnny_Trucker:Notify",source,"sucesso",Lang[Config.lang]['hired'])
				openUI(source,true)
			else
				TriggerClientEvent("Johnny_Trucker:Notify",source,"negado","Dinheiro insuficiente")
			end
		else
			TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['max_drivers'])
		end
	end
end)
...
AddEventHandler("truck_logistics:fireDriver",function(driver_id)
	...
	if user_id then
		...
		local sql = "UPDATE `trucker_trucks` SET driver = NULL WHERE driver = @driver_id";
		MySQL.Sync.execute(sql, {['@driver_id'] = driver_id});
		TriggerClientEvent("Johnny_Trucker:Notify",source,"sucesso",Lang[Config.lang]['fired'])
		openUI(source,true)
	end
end)
		...
		if user_id then
			...
			if not query or not query.remaining_amount or query.remaining_amount <= 0 then
				...
				if amount and amount > 0 then
					...
					TriggerClientEvent("Johnny_Trucker:Notify",source,"sucesso",Lang[Config.lang]['money_withdrawn'])
					openUI(source,true)
				else
					TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['insufficiente_money'])
				end
			else
				TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['pay_loans'])
			end
		end
		debug_cooldown[source] = nil
	end
	...
	if user_id then
		...
		if amount and amount > 0 then
			...
			if money >= amount then
				...
				giveTruckerMoney(user_id,amount)
				TriggerClientEvent("Johnny_Trucker:Notify",source,"sucesso",Lang[Config.lang]['money_deposited'])
				openUI(source,true)
			else
				TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['insufficiente_money'])
			end
		else
			TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['invalid_value'])
		end
	end
end)
	...
	if debug_cooldown[source] == nil then
		...
		if user_id then
			...
			if amount_loans + Config.emprestimos.valores[data.loan_id][1] <= getMaxEmprestimo(user_id) then
				...
				TriggerClientEvent("Johnny_Trucker:Notify",source,"sucesso",Lang[Config.lang]['loan'])
				openUI(source,true)
			else
				TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['no_loan'])
			end
		end
		debug_cooldown[source] = nil
	end
...
AddEventHandler("truck_logistics:payLoan",function(data)
	...
	if user_id then
		...
		if tryGetTruckerMoney(user_id,query.remaining_amount) then
			...
			TriggerClientEvent("Johnny_Trucker:Notify",source,"sucesso",Lang[Config.lang]['loan_paid'])
			openUI(source,true)
		else
			TriggerClientEvent("Johnny_Trucker:Notify",source,"negado",Lang[Config.lang]['insufficiente_funds'])
		end
...
AddEventHandler("truck_logistics:finishJob",function(data,distance,reward,truck_data,truck_engine,truck_body,trailer_body)
	...
	if user_id then
		...
		TriggerClientEvent("Johnny_Trucker:Notify",source,"sucesso",Lang[Config.lang]['reward']:format(tostring(received_amount),tostring(trailer_body*100),tostring(received_xp)))
