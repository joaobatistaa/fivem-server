window.addEventListener('message', function (event) {
	...
	if (item.showmenu){
		...
		$('#new-contracts-1').append('Novos trabalhos a cada ' + (config.cooldown*2) + ' min');
		$('#new-contracts-2').empty();
		$('#new-contracts-2').append('Novos trabalhos a cada ' + (config.cooldown*2) + ' min');
		...
		for (const contract of contracts) {
					...
					<div class="d-flex flex-row align-items-center"><i class="fa fa-check-circle checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								...
								<div class="ml-3"><i class="fas fa-coins"></i><span class="ml-2">Valor Ganho: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(contract.reward) + `</span></div>
							</div>
						</div>
					</div>
			...
			list_item += `
						...
						<button onclick="startJob(` + contract.contract_id + `,` + contract.reward + `,` + contract.distance + `)" class="btn btn-blue btn-darken-2 white">Iniciar Trabalho</button>
					</div>
				</li>
			</ul>
			`;
		...
		$('#skills-desc').empty();
		$('#skills-desc').append("Atualiza as tuas habilidades para conseguir trabalhos melhores (Pontos de habilidade disponíveis: " + users.skill_points + ")");
		...
		list_item = `
			<div class="col-12 mt-3 mb-1">
				...
				<p>Compra mais camiões para ti e para os teus motoristas</p>
			</div>`;
			...
			list_item += `
				<div class="card"> <img src="` + truck.img + `" class="card-img-top" width="100%">
					<div class="card-body pt-0 px-0">
						<div class="d-flex flex-row justify-content-between mb-0 mt-3 px-3"> <span class="text-muted">Camião</span>
							<h6>` + truck.name + `</h6>
						</div>
						<hr class="mt-2 mx-3">
						...
						<div class="mx-3 mt-3 mb-2"><button onclick="buyTruck('` + key + `','` + truck.price + `')" type="button" class="btn btn-blue btn-darken-2 white btn-block"><small>COMPRAR</small></button></div> <small class="d-flex justify-content-center text-muted">*Contrato Legal</small>
					</div>
				</div>
				`;
		}
		...
		for (const truck of myTrucks) {
			...
			if(truck.driver == 0){
				...
				$('#diagnostic-body').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.body + ` %</h3><span>Reparar Chassi (` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format((100-truck.body)*config.valor_reparo.body) + `)</span>
						</div>
				...
				$('#diagnostic-engine').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.engine + ` %</h3><span>Reparar Motor (` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format((100-truck.engine)*config.valor_reparo.engine) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				$('#diagnostic-transmission').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.transmission + ` %</h3><span>Reparar Transmissão (` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format((100-truck.transmission)*config.valor_reparo.transmission) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
				...
				$('#diagnostic-wheels').append(`
					<div class="media d-flex">
						<div class="media-body text-left">
							<h3 class="` + color + `">` + truck.wheels + ` %</h3><span>Reparar Rodas (` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format((100-truck.wheels)*config.valor_reparo.wheels) + `)</span>
						</div>
						<div class="align-self-center">
							<i class="fas fa-tools ` + color + ` font-large-2 float-right"></i>
						</div>
			...
			list_item += `
				<li class="d-flex justify-content-between">
					<div class="d-flex flex-row align-items-center"><img src="` + config.concessionaria[truck.truck_name].img + `" class="img-radius img-width" alt="User-Profile-Image">
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								...
								<div class="ml-3">
									<i class="fas fa-tools"></i><span class="ml-2">Transmissãoo: ` + truck.transmission + `%</span>
								</div>
								<div class="ml-3">
					...
					<div class="d-flex flex-row align-items-center">
						...
						<button onclick="sellTruck('` + truck.truck_id + `','` + truck.truck_name + `')" class="btn btn-red btn-accent-4 white">Vender Camião</button>
					</div>
				</li>
				`;
		}
		...
		list_item = `
			<div class="col-12 mt-3 mb-1">
				...
				<p>Recruta novos condutores para trabalharem na tua empresa</p>
			</div>
			`;
		for (const driver of drivers) {
			if (driver.user_id == null || driver.user_id == undefined){
				list_item += `
					<div class="card user-card">
						<div class="card-block">
							...
							<hr>
							<p class="text-muted">Tipos de Produto</p>
							...
							<p class="text-muted">Carga Valiosa</p>
							<ul class="list-unstyled activity-leval">
							...
							<p class="text-muted">Carga a Tempo</p>
							<ul class="list-unstyled activity-leval">
				...
				$('#drivers-page-list').append(`
					<li class="d-flex justify-content-between">
						<div class="d-flex flex-row align-items-center">
							...
							<div class="ml-2">
								...
								<div class="d-flex flex-row mt-1 text-black-50 date-time">
									...
									<div class="ml-3">
										<i class="fas fa-medal"></i><span class="ml-2">Habilidades do Condutor: Tipos de Produto (` + driver.product_type + `) Distância (` + driver.distance + `) Carga Valiosa (` + driver.valuable + `) Carga Frágil (` + driver.fragile + `) Entrega a Tempo (` + driver.fast + `)</span>
									</div>
								</div>
							</div>
						</div>
						<div class="d-flex flex-row align-items-center">
							...
							<button onclick="fireDriver('` + driver.driver_id + `')" class="btn btn-red btn-accent-4 white">Despedir Condutor</button>
						</div>
					</li>
				`);
			}
		...
		$('#loan-title').empty();
		$('#loan-title').append('Pede empréstimos para investir na tua empresa (Máximo de empréstimos ativos: ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.max_emprestimo) + ')');
		
		$('#loan-1').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[0][0]) + '</h3><span>Pagas ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[0][1]) + ' por dia</span>';
		$('#loan-1').append(list_item);
		$('#loan-2').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[1][0]) + '</h3><span>Pagas ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[1][1]) + ' por dia</span>';
		$('#loan-2').append(list_item);
		$('#loan-3').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[2][0]) + '</h3><span>Pagas ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[2][1]) + ' por dia</span>';
		$('#loan-3').append(list_item);
		$('#loan-4').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[3][0]) + '</h3><span>Pagas ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[3][1]) + ' por dia</span>';
				...
				<li class="d-flex justify-content-between">
					<div class="d-flex flex-row align-items-center"><i class="fa fa-check-circle checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								<div><i class="fas fa-coins"></i></i><span class="ml-2">Em Dívida: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(loan.remaining_amount) + `</span></div>
								<div class="ml-3"><i class="fas fa-coins"></i></i><span class="ml-2">Valor Diário: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(loan.day_cost) + `</span></div>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center">
						<button onclick="payLoan('` + loan.id + `')" class="btn btn-red btn-accent-4 white">Pagar Empréstimo</button>
					</div>
				</li>
			`;
...
function getDriverAvailableTrucksHTML(myTrucks,driver,config){
	...
	if (has_truck == null) {
		html = '<option selected="selected">Selecionar Camião</option>' + html;
	}else{
		html = '<option driver_id="' + driver.driver_id + '">Selecionar Camião</option>' + html;
	}
	return html;
}

function getMyTruckHTML(truck){
	return truck.driver==0 ? `<button onclick="spawnTruck(` + truck.truck_id + `)" class="btn btn-blue btn-darken-2 white white mr-2">Conduzir Camião</button> <button onclick="setDriver(null,'` + truck.truck_id + `')" class="btn btn-blue btn-darken-2 white white mr-2">Retirar Seleção</button>` : `<button onclick="setDriver('0','` + truck.truck_id + `')" class="btn btn-blue btn-darken-2 white white mr-2">Selecionar Camião</button>`
}
...
function post(name,data){
	$.post("http://Johnny_Trucker/"+name,JSON.stringify(data),function(datab){
		if (datab != "ok"){
			console.log(datab);
		}
	});
