					...
					if timeElapsed > data.jobItem[1].time then
						...
						for k,v in ipairs(data.jobItem) do
							...
							if v.name ~= _U('delivery') and itemQtty >= v.max then
								xPlayer.showNotification(_U('max_limit', xPlayer.getInventoryItem(v.db_name).label))
							...
							elseif v.requires ~= 'nothing' and requiredItemQtty < v.remove then
								...
								if v.name ~= _U('delivery') then
									...
									if v.requires ~= 'nothing' then
										xPlayer.removeInventoryItem(v.requires, v.remove)
