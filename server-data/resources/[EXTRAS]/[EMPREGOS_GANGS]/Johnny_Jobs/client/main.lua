...
AddEventHandler('esx_jobs:spawnJobVehicle', function(spawnPoint, vehicle)
	...
	ESX.Game.SpawnVehicle(vehicle.Hash, spawnPoint.Pos, spawnPoint.Heading, function(spawnedVehicle)
		...
		--TriggerServerEvent("johnny:server:darChaves", plate, vehicle.Hash)
		TaskWarpPedIntoVehicle(playerPed, spawnedVehicle, -1)
		exports['qs-vehiclekeys']:GiveKeysAuto()
		--TriggerServerEvent('vehiclekeys:server:givekey', plate, vehicle.Hash)
	...
	while true do
		...
		if PlayerData.job then
			...
			for k,v in pairs(zones) do
				...
				if onDuty or v.Type == 'cloakroom' or PlayerData.job.name == 'jornalista' or PlayerData.job.name == 'mineiro' then
			...
			if IsControlJustReleased(0, 38) and not menuIsShowed and isInMarker then
				if onDuty or currentZone.Type == 'cloakroom' or PlayerData.job.name == 'jornalista' or PlayerData.job.name == 'mineiro' then
					TriggerEvent('esx_jobs:action', PlayerData.job.name, currentZone, currentZoneIndex)
					if currentZone.Type == 'wash_stone' then
			...
			if isInMarker and not menuIsShowed then
				hintIsShowed = true
				if (onDuty or currentZone.Type == 'cloakroom' or PlayerData.job.name == 'jornalista' or PlayerData.job.name == 'mineiro') and currentZone.Type ~= 'vehdelete' then
					hintToDisplay = currentZone.Hint
					hintIsShowed = true
				elseif currentZone.Type == 'vehdelete' and (onDuty or PlayerData.job.name == 'jornalista' or PlayerData.job.name == 'mineiro') then
					local playerPed = PlayerPedId()
