...
local HasAlreadyEnteredMarker_bahamas = false
local LastStation_bahamas     = nil
local LastPart                = nil
local LastPartNum_bahamas     = nil
local CurrentAction_bahamas           = nil
local CurrentActionMsg_bahamas        = ''
local CurrentActionData_bahamas       = {}
local isDead_bahamas                  = false
...
function Garagem_Bahamas(station, partNum)		
	local foundSpawnPoint, spawnPoint = GetAvailable_SpawnPoint_Bahamas(station, partNum)
	if foundSpawnPoint then
		TriggerEvent("johnny_garagens:carroempresa", spawnPoint, 'bahamas')
	end 
end

function GetAvailable_SpawnPoint_Bahamas(station, partNum)
	local spawnPoints = Config.bahamasStations[station].Veiculos[partNum].SpawnPoints
...
AddEventHandler('empresa_bahamas:hasEnteredMarker', function(station, part, partNum)

	if part == 'Vestiarios' then
	
		CurrentAction_bahamas     = 'menu_vestiario'
		
	elseif part == 'Cofres' then
	
		CurrentAction_bahamas     = 'menu_cofre'
		CurrentActionData_bahamas = {station = station}
		
	elseif part == 'VeiculosSpawner' then

		CurrentAction_bahamas     = 'retirar_veiculo'
		CurrentActionData_bahamas = {station = station, partNum = partNum}
	...
	elseif part == 'GuardarVeiculos' then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			if DoesEntityExist(vehicle) then
				CurrentAction_bahamas     = 'guardar_veiculo'
				CurrentActionData_bahamas = {vehicle = vehicle}
			end
		end

	elseif part == 'GestaoEmpresa' then
	
		CurrentAction_bahamas     = 'menu_gestao_empresa'
		
	end

end)

AddEventHandler('empresa_bahamas:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction_bahamas = nil
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'bahamas' then
			...
			if isInMarker or (isInMarker and (LastStation_bahamas ~= currentStation or LastPart_bahamas ~= currentPart or LastPartNum_bahamas ~= currentPartNum)) then
				sleep = 1
				if
					(LastStation_bahamas ~= nil and LastPart_bahamas ~= nil and LastPartNum_bahamas ~= nil) and
					(LastStation_bahamas ~= currentStation or LastPart_bahamas ~= currentPart or LastPartNum_bahamas ~= currentPartNum)
				then
					TriggerEvent('empresa_bahamas:hasExitedMarker', LastStation_bahamas, LastPart_bahamas, LastPartNum_bahamas)
					hasExited = true
				end

				HasAlreadyEnteredMarker_bahamas = true
				LastStation_bahamas             = currentStation
				LastPart_bahamas                = currentPart
				LastPartNum_bahamas             = currentPartNum

				TriggerEvent('empresa_bahamas:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end

			if not hasExited and not isInMarker and HasAlreadyEnteredMarker_bahamas then
				sleep = 1
				HasAlreadyEnteredMarker_bahamas = false
				TriggerEvent('empresa_bahamas:hasExitedMarker', LastStation_bahamas, LastPart_bahamas, LastPartNum_bahamas)
			end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'bahamas' and not isDead_bahamas then
			local playerPed      = PlayerPedId()
			local carro = IsPedInAnyVehicle(playerPed,  false)
			
			if CurrentAction_bahamas ~= nil then

				if IsControlJustReleased(0, Keys['E']) then

					if CurrentAction_bahamas == 'menu_vestiario' then
					
						MenuRoupa()
						
					elseif CurrentAction_bahamas == 'menu_cofre' then
					
						local other = {}
						other.maxweight = 999999999 -- Custom weight statsh.
						other.slots = 200 -- Custom slots spaces.
						TriggerServerEvent("inventory:server:OpenInventory", "stash", "bahamas", other)
						TriggerEvent("inventory:client:SetCurrentStash", "bahamas")
						
					elseif CurrentAction_bahamas == 'retirar_veiculo' then
						...
						if carro then
							...
							Garagem_Bahamas(CurrentActionData_bahamas.station, CurrentActionData_bahamas.partNum)
						end
						
					elseif CurrentAction_bahamas == 'guardar_veiculo' then
						
						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData_bahamas.vehicle)
						...
						ESX.TriggerServerCallback('johnny_garagens:getCarrosEmpresa',function(valid)	
							if (valid) then
								...
								TaskLeaveVehicle(playerPed, CurrentActionData_bahamas.vehicle, 0)
								Citizen.Wait(2500)
								ESX.Game.DeleteVehicle(CurrentActionData_bahamas.vehicle)
					...
					elseif CurrentAction_bahamas == 'menu_gestao_empresa' then
			...
			if IsControlJustReleased(0, Keys['F6']) and not isDead_bahamas and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'bahamas_actions') then
				MenuF6_Bahamas()
			end
