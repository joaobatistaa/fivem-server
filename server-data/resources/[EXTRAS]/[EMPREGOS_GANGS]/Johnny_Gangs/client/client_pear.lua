...
local HasAlreadyEnteredMarker_pear = false
local LastStation_pear     = nil
local LastPart                = nil
local LastPartNum_pear     = nil
local CurrentAction_pear           = nil
local CurrentActionMsg_pear        = ''
local CurrentActionData_pear       = {}
local isDead_pear                  = false
...
function Garagem_pear(station, partNum)		
	local foundSpawnPoint, spawnPoint = GetAvailable_SpawnPoint_pear(station, partNum)
	if foundSpawnPoint then
		TriggerEvent("johnny_garagens:carroempresa", spawnPoint, 'pear')
	end 
end

function GetAvailable_SpawnPoint_pear(station, partNum)
	local spawnPoints = Config.pearStations[station].Veiculos[partNum].SpawnPoints
...
function MenuF6_pear()
	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pear_actions',
	{
		title    = '🍽️ Menu Pear',
		align    = 'top-left',
		elements = {
...
AddEventHandler('empresa_pear:hasEnteredMarker', function(station, part, partNum)

	if part == 'Vestiarios' then
	
		CurrentAction_pear     = 'menu_vestiario'
		
	elseif part == 'Cofres' then
	
		CurrentAction_pear     = 'menu_cofre'
		CurrentActionData_pear = {station = station}
		
	elseif part == 'VeiculosSpawner' then

		CurrentAction_pear     = 'retirar_veiculo'
		CurrentActionData_pear = {station = station, partNum = partNum}
	...
	elseif part == 'GuardarVeiculos' then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			if DoesEntityExist(vehicle) then
				CurrentAction_pear     = 'guardar_veiculo'
				CurrentActionData_pear = {vehicle = vehicle}
			end
		end

	elseif part == 'GestaoEmpresa' then
	
		CurrentAction_pear     = 'menu_gestao_empresa'
		
	end

end)

AddEventHandler('empresa_pear:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction_pear = nil
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'pear' then

			local playerPed = PlayerPedId()
			local coords    = GetEntityCoords(playerPed)

			for k,v in pairs(Config.pearStations) do
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'pear' then
			...
			if isInMarker or (isInMarker and (LastStation_pear ~= currentStation or LastPart_pear ~= currentPart or LastPartNum_pear ~= currentPartNum)) then
				sleep = 1
				if
					(LastStation_pear ~= nil and LastPart_pear ~= nil and LastPartNum_pear ~= nil) and
					(LastStation_pear ~= currentStation or LastPart_pear ~= currentPart or LastPartNum_pear ~= currentPartNum)
				then
					TriggerEvent('empresa_pear:hasExitedMarker', LastStation_pear, LastPart_pear, LastPartNum_pear)
					hasExited = true
				end

				HasAlreadyEnteredMarker_pear = true
				LastStation_pear             = currentStation
				LastPart_pear                = currentPart
				LastPartNum_pear             = currentPartNum

				TriggerEvent('empresa_pear:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end

			if not hasExited and not isInMarker and HasAlreadyEnteredMarker_pear then
				sleep = 1
				HasAlreadyEnteredMarker_pear = false
				TriggerEvent('empresa_pear:hasExitedMarker', LastStation_pear, LastPart_pear, LastPartNum_pear)
			end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'pear' and not isDead_pear then
			local playerPed      = PlayerPedId()
			local carro = IsPedInAnyVehicle(playerPed,  false)
			
			if CurrentAction_pear ~= nil then

				if IsControlJustReleased(0, Keys['E']) then

					if CurrentAction_pear == 'menu_vestiario' then
					
						MenuRoupa()
						
					elseif CurrentAction_pear == 'menu_cofre' then
					
						local other = {}
						other.maxweight = 999999999 -- Custom weight statsh.
						other.slots = 200 -- Custom slots spaces.
						TriggerServerEvent("inventory:server:OpenInventory", "stash", "pear", other)
						TriggerEvent("inventory:client:SetCurrentStash", "pear")
						
					elseif CurrentAction_pear == 'retirar_veiculo' then
						...
						if carro then
							...
							Garagem_pear(CurrentActionData_pear.station, CurrentActionData_pear.partNum)
						end
						
					elseif CurrentAction_pear == 'guardar_veiculo' then
				
						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData_pear.vehicle)
						...
						ESX.TriggerServerCallback('johnny_garagens:getCarrosEmpresa',function(valid)	
							if (valid) then
								...
								TaskLeaveVehicle(playerPed, CurrentActionData_pear.vehicle, 0)
								Citizen.Wait(2500)
								ESX.Game.DeleteVehicle(CurrentActionData_pear.vehicle)
					...
					elseif CurrentAction_pear == 'menu_gestao_empresa' then
					
						ESX.UI.Menu.CloseAll()
						TriggerEvent('esx_society:openBossMenu', 'pear', function(data, menu)
							menu.close()
							CurrentAction_pear     = 'menu_gestao_empresa'
			...
			if IsControlJustReleased(0, Keys['F6']) and not isDead_pear and PlayerData.job ~= nil and PlayerData.job.name == 'pear' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'pear_actions') then
				MenuF6_pear()
			end
