...
local HasAlreadyEnteredMarker_grove = false
local LastStation_grove     = nil
local LastPart                = nil
local LastPartNum_grove     = nil
local CurrentAction_grove           = nil
local CurrentActionMsg_grove        = ''
local CurrentActionData_grove       = {}
local isDead_grove                  = false
...
function Garagem_Grove(station, partNum)		
	local foundSpawnPoint, spawnPoint = GetAvailable_SpawnPoint_Grove(station, partNum)
	if foundSpawnPoint then
		TriggerEvent("johnny_garagens:carroempresa", spawnPoint, 'grove')
	end 
end

function GetAvailable_SpawnPoint_Grove(station, partNum)
	local spawnPoints = Config.groveStations[station].Veiculos[partNum].SpawnPoints
...
AddEventHandler('empresa_grove:hasEnteredMarker', function(station, part, partNum)

	if part == 'Vestiarios' then
	
		CurrentAction_grove     = 'menu_vestiario'
		
	elseif part == 'Cofres' then
	
		CurrentAction_grove     = 'menu_cofre'
		CurrentActionData_grove = {station = station}
		
	elseif part == 'VeiculosSpawner' then

		CurrentAction_grove     = 'retirar_veiculo'
		CurrentActionData_grove = {station = station, partNum = partNum}
	...
	elseif part == 'GuardarVeiculos' then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			if DoesEntityExist(vehicle) then
				CurrentAction_grove     = 'guardar_veiculo'
				CurrentActionData_grove = {vehicle = vehicle}
			end
		end

	elseif part == 'GestaoEmpresa' then
	
		CurrentAction_grove     = 'menu_gestao_empresa'
		
	end

end)

AddEventHandler('empresa_grove:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction_grove = nil
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'grove' then
			...
			if isInMarker or (isInMarker and (LastStation_grove ~= currentStation or LastPart_grove ~= currentPart or LastPartNum_grove ~= currentPartNum)) then
				sleep = 1
				if
					(LastStation_grove ~= nil and LastPart_grove ~= nil and LastPartNum_grove ~= nil) and
					(LastStation_grove ~= currentStation or LastPart_grove ~= currentPart or LastPartNum_grove ~= currentPartNum)
				then
					TriggerEvent('empresa_grove:hasExitedMarker', LastStation_grove, LastPart_grove, LastPartNum_grove)
					hasExited = true
				end

				HasAlreadyEnteredMarker_grove = true
				LastStation_grove             = currentStation
				LastPart_grove                = currentPart
				LastPartNum_grove             = currentPartNum

				TriggerEvent('empresa_grove:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end

			if not hasExited and not isInMarker and HasAlreadyEnteredMarker_grove then
				sleep = 1
				HasAlreadyEnteredMarker_grove = false
				TriggerEvent('empresa_grove:hasExitedMarker', LastStation_grove, LastPart_grove, LastPartNum_grove)
			end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'grove' and not isDead_grove then
			local playerPed      = PlayerPedId()
			local carro = IsPedInAnyVehicle(playerPed,  false)
			
			if CurrentAction_grove ~= nil then

				if IsControlJustReleased(0, Keys['E']) then

					if CurrentAction_grove == 'menu_vestiario' then
					
						MenuRoupa()
						
					elseif CurrentAction_grove == 'menu_cofre' then
					
						local other = {}
						other.maxweight = 999999999 -- Custom weight statsh.
						other.slots = 200 -- Custom slots spaces.
						TriggerServerEvent("inventory:server:OpenInventory", "stash", "grove", other)
						TriggerEvent("inventory:client:SetCurrentStash", "grove")
						
					elseif CurrentAction_grove == 'retirar_veiculo' then
						...
						if carro then
							...
							Garagem_Grove(CurrentActionData_grove.station, CurrentActionData_grove.partNum)
						end
						
					elseif CurrentAction_grove == 'guardar_veiculo' then

						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData_grove.vehicle)
						...
						ESX.TriggerServerCallback('johnny_garagens:getCarrosEmpresa',function(valid)	
							if (valid) then
								...
								TaskLeaveVehicle(playerPed, CurrentActionData_grove.vehicle, 0)
								Citizen.Wait(2500)
								ESX.Game.DeleteVehicle(CurrentActionData_grove.vehicle)
					...
					elseif CurrentAction_grove == 'menu_gestao_empresa' then
			...
			if IsControlJustReleased(0, Keys['F6']) and not isDead_grove and PlayerData.job ~= nil and PlayerData.job.name == 'grove' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'grove_actions') then
				MenuF6_Grove()
			end
