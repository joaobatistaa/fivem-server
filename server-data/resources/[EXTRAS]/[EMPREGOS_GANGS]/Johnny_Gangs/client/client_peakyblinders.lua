...
local HasAlreadyEnteredMarker_peakyblinders = false
local LastStation_peakyblinders     = nil
local LastPart                = nil
local LastPartNum_peakyblinders     = nil
local CurrentAction_peakyblinders           = nil
local CurrentActionMsg_peakyblinders        = ''
local CurrentActionData_peakyblinders       = {}
local isDead_peakyblinders                  = false
...
function Garagem_PeakyBlinders(station, partNum)		
	local foundSpawnPoint, spawnPoint = GetAvailable_SpawnPoint_PeakyBlinders(station, partNum)
	if foundSpawnPoint then
		TriggerEvent("johnny_garagens:carroempresa", spawnPoint, 'peakyblinders')
	end 
end

function GetAvailable_SpawnPoint_PeakyBlinders(station, partNum)
	local spawnPoints = Config.peakyblindersStations[station].Veiculos[partNum].SpawnPoints
...
function MenuF6_PeakyBlinders()
	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'peakyblinders_actions',
	{
		title    = '👺 Menu PeakyBlinders',
		align    = 'top-left',
		elements = {
...
AddEventHandler('empresa_peakyblinders:hasEnteredMarker', function(station, part, partNum)

	if part == 'Vestiarios' then
	
		CurrentAction_peakyblinders     = 'menu_vestiario'
		
	elseif part == 'Cofres' then
	
		CurrentAction_peakyblinders     = 'menu_cofre'
		CurrentActionData_peakyblinders = {station = station}
		
	elseif part == 'VeiculosSpawner' then

		CurrentAction_peakyblinders     = 'retirar_veiculo'
		CurrentActionData_peakyblinders = {station = station, partNum = partNum}
	...
	elseif part == 'GuardarVeiculos' then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			if DoesEntityExist(vehicle) then
				CurrentAction_peakyblinders     = 'guardar_veiculo'
				CurrentActionData_peakyblinders = {vehicle = vehicle}
			end
		end

	elseif part == 'GestaoEmpresa' then
	
		CurrentAction_peakyblinders     = 'menu_gestao_empresa'
		
	end

end)

AddEventHandler('empresa_peakyblinders:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction_peakyblinders = nil
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'peakyblinders' then

			local playerPed = PlayerPedId()
			local coords    = GetEntityCoords(playerPed)

			for k,v in pairs(Config.peakyblindersStations) do
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'peakyblinders' then
			...
			if isInMarker or (isInMarker and (LastStation_peakyblinders ~= currentStation or LastPart_peakyblinders ~= currentPart or LastPartNum_peakyblinders ~= currentPartNum)) then
				sleep = 1
				if
					(LastStation_peakyblinders ~= nil and LastPart_peakyblinders ~= nil and LastPartNum_peakyblinders ~= nil) and
					(LastStation_peakyblinders ~= currentStation or LastPart_peakyblinders ~= currentPart or LastPartNum_peakyblinders ~= currentPartNum)
				then
					TriggerEvent('empresa_peakyblinders:hasExitedMarker', LastStation_peakyblinders, LastPart_peakyblinders, LastPartNum_peakyblinders)
					hasExited = true
				end

				HasAlreadyEnteredMarker_peakyblinders = true
				LastStation_peakyblinders             = currentStation
				LastPart_peakyblinders                = currentPart
				LastPartNum_peakyblinders             = currentPartNum

				TriggerEvent('empresa_peakyblinders:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end

			if not hasExited and not isInMarker and HasAlreadyEnteredMarker_peakyblinders then
				sleep = 1
				HasAlreadyEnteredMarker_peakyblinders = false
				TriggerEvent('empresa_peakyblinders:hasExitedMarker', LastStation_peakyblinders, LastPart_peakyblinders, LastPartNum_peakyblinders)
			end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'peakyblinders' and not isDead_peakyblinders then
			local playerPed      = PlayerPedId()
			local carro = IsPedInAnyVehicle(playerPed,  false)
			
			if CurrentAction_peakyblinders ~= nil then

				if IsControlJustReleased(0, Keys['E']) then

					if CurrentAction_peakyblinders == 'menu_vestiario' then
					
						MenuRoupa()
						
					elseif CurrentAction_peakyblinders == 'menu_cofre' then
					
						local other = {}
						other.maxweight = 999999999 -- Custom weight statsh.
						other.slots = 200 -- Custom slots spaces.
						TriggerServerEvent("inventory:server:OpenInventory", "stash", "peakyblinders", other)
						TriggerEvent("inventory:client:SetCurrentStash", "peakyblinders")
						
					elseif CurrentAction_peakyblinders == 'retirar_veiculo' then
						...
						if carro then
							...
							Garagem_PeakyBlinders(CurrentActionData_peakyblinders.station, CurrentActionData_peakyblinders.partNum)
						end
						
					elseif CurrentAction_peakyblinders == 'guardar_veiculo' then
			
						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData_peakyblinders.vehicle)
						...
						ESX.TriggerServerCallback('johnny_garagens:getCarrosEmpresa',function(valid)	
							if (valid) then
								...
								TaskLeaveVehicle(playerPed, CurrentActionData_peakyblinders.vehicle, 0)
								Citizen.Wait(2500)
								ESX.Game.DeleteVehicle(CurrentActionData_peakyblinders.vehicle)
					...
					elseif CurrentAction_peakyblinders == 'menu_gestao_empresa' then
					
						ESX.UI.Menu.CloseAll()
						TriggerEvent('esx_society:openBossMenu', 'peakyblinders', function(data, menu)
							menu.close()
							CurrentAction_peakyblinders     = 'menu_gestao_empresa'
			...
			if IsControlJustReleased(0, Keys['F6']) and not isDead_peakyblinders and PlayerData.job ~= nil and PlayerData.job.name == 'peakyblinders' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'peakyblinders_actions') then
				MenuF6_PeakyBlinders()
			end
...
AddEventHandler('esx:onPlayerDeath', function(data)
	isDead_peakyblinders = true
end)

AddEventHandler('playerSpawned', function(spawn)
	isDead_peakyblinders = false
end)