...
local HasAlreadyEnteredMarker_vagos = false
local LastStation_vagos     = nil
local LastPart                = nil
local LastPartNum_vagos     = nil
local CurrentAction_vagos           = nil
local CurrentActionMsg_vagos        = ''
local CurrentActionData_vagos       = {}
local isDead_vagos                  = false
...
function Garagem_Vagos(station, partNum)		
	local foundSpawnPoint, spawnPoint = GetAvailable_SpawnPoint_Vagos(station, partNum)
	if foundSpawnPoint then
		TriggerEvent("johnny_garagens:carroempresa", spawnPoint, 'vagos')
	end 
end

function GetAvailable_SpawnPoint_Vagos(station, partNum)
	local spawnPoints = Config.vagosStations[station].Veiculos[partNum].SpawnPoints
...
AddEventHandler('empresa_vagos:hasEnteredMarker', function(station, part, partNum)

	if part == 'Vestiarios' then
	
		CurrentAction_vagos     = 'menu_vestiario'
		
	elseif part == 'Cofres' then
	
		CurrentAction_vagos     = 'menu_cofre'
		CurrentActionData_vagos = {station = station}
		
	elseif part == 'VeiculosSpawner' then

		CurrentAction_vagos     = 'retirar_veiculo'
		CurrentActionData_vagos = {station = station, partNum = partNum}
	...
	elseif part == 'GuardarVeiculos' then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			if DoesEntityExist(vehicle) then
				CurrentAction_vagos     = 'guardar_veiculo'
				CurrentActionData_vagos = {vehicle = vehicle}
			end
		end

	elseif part == 'GestaoEmpresa' then
	
		CurrentAction_vagos     = 'menu_gestao_empresa'
		
	end

end)

AddEventHandler('empresa_vagos:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction_vagos = nil
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'vagos' then
			...
			if isInMarker or (isInMarker and (LastStation_vagos ~= currentStation or LastPart_vagos ~= currentPart or LastPartNum_vagos ~= currentPartNum)) then
				sleep = 1
				if
					(LastStation_vagos ~= nil and LastPart_vagos ~= nil and LastPartNum_vagos ~= nil) and
					(LastStation_vagos ~= currentStation or LastPart_vagos ~= currentPart or LastPartNum_vagos ~= currentPartNum)
				then
					TriggerEvent('empresa_vagos:hasExitedMarker', LastStation_vagos, LastPart_vagos, LastPartNum_vagos)
					hasExited = true
				end

				HasAlreadyEnteredMarker_vagos = true
				LastStation_vagos             = currentStation
				LastPart_vagos                = currentPart
				LastPartNum_vagos             = currentPartNum

				TriggerEvent('empresa_vagos:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end

			if not hasExited and not isInMarker and HasAlreadyEnteredMarker_vagos then
				sleep = 1
				HasAlreadyEnteredMarker_vagos = false
				TriggerEvent('empresa_vagos:hasExitedMarker', LastStation_vagos, LastPart_vagos, LastPartNum_vagos)
			end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'vagos' and not isDead_vagos then
			local playerPed      = PlayerPedId()
			local carro = IsPedInAnyVehicle(playerPed,  false)
			
			if CurrentAction_vagos ~= nil then

				if IsControlJustReleased(0, Keys['E']) then

					if CurrentAction_vagos == 'menu_vestiario' then
					
						MenuRoupa()
						
					elseif CurrentAction_vagos == 'menu_cofre' then
					
						local other = {}
						other.maxweight = 999999999 -- Custom weight statsh.
						other.slots = 200 -- Custom slots spaces.
						TriggerServerEvent("inventory:server:OpenInventory", "stash", "vagos", other)
						TriggerEvent("inventory:client:SetCurrentStash", "vagos")
						
					elseif CurrentAction_vagos == 'retirar_veiculo' then
						...
						if carro then
							...
							Garagem_Vagos(CurrentActionData_vagos.station, CurrentActionData_vagos.partNum)
						end
						
					elseif CurrentAction_vagos == 'guardar_veiculo' then
				
						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData_vagos.vehicle)
						...
						ESX.TriggerServerCallback('johnny_garagens:getCarrosEmpresa',function(valid)	
							if (valid) then
								...
								TaskLeaveVehicle(playerPed, CurrentActionData_vagos.vehicle, 0)
								Citizen.Wait(2500)
								ESX.Game.DeleteVehicle(CurrentActionData_vagos.vehicle)
					...
					elseif CurrentAction_vagos == 'menu_gestao_empresa' then
			...
			if IsControlJustReleased(0, Keys['F6']) and not isDead_vagos and PlayerData.job ~= nil and PlayerData.job.name == 'vagos' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'vagos_actions') then
				MenuF6_Vagos()
			end
