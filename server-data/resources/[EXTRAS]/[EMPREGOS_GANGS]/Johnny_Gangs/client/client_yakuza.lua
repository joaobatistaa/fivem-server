...
local HasAlreadyEnteredMarker_yakuza = false
local LastStation_yakuza     = nil
local LastPart                = nil
local LastPartNum_yakuza     = nil
local CurrentAction_yakuza           = nil
local CurrentActionMsg_yakuza        = ''
local CurrentActionData_yakuza       = {}
local isDead_yakuza                  = false
...
function Garagem_Yakuza(station, partNum)		
	local foundSpawnPoint, spawnPoint = GetAvailable_SpawnPoint_Yakuza(station, partNum)
	if foundSpawnPoint then
		TriggerEvent("johnny_garagens:carroempresa", spawnPoint, 'yakuza')
	end 
end

function GetAvailable_SpawnPoint_Yakuza(station, partNum)
	local spawnPoints = Config.yakuzaStations[station].Veiculos[partNum].SpawnPoints
...
AddEventHandler('empresa_yakuza:hasEnteredMarker', function(station, part, partNum)

	if part == 'Vestiarios' then
	
		CurrentAction_yakuza     = 'menu_vestiario'
		
	elseif part == 'Cofres' then
	
		CurrentAction_yakuza     = 'menu_cofre'
		CurrentActionData_yakuza = {station = station}
		
	elseif part == 'VeiculosSpawner' then

		CurrentAction_yakuza     = 'retirar_veiculo'
		CurrentActionData_yakuza = {station = station, partNum = partNum}
	...
	elseif part == 'GuardarVeiculos' then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			if DoesEntityExist(vehicle) then
				CurrentAction_yakuza     = 'guardar_veiculo'
				CurrentActionData_yakuza = {vehicle = vehicle}
			end
		end

	elseif part == 'GestaoEmpresa' then
	
		CurrentAction_yakuza     = 'menu_gestao_empresa'
		
	end

end)

AddEventHandler('empresa_yakuza:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction_yakuza = nil
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'yakuza' then
			...
			if isInMarker or (isInMarker and (LastStation_yakuza ~= currentStation or LastPart_yakuza ~= currentPart or LastPartNum_yakuza ~= currentPartNum)) then
				sleep = 1
				if
					(LastStation_yakuza ~= nil and LastPart_yakuza ~= nil and LastPartNum_yakuza ~= nil) and
					(LastStation_yakuza ~= currentStation or LastPart_yakuza ~= currentPart or LastPartNum_yakuza ~= currentPartNum)
				then
					TriggerEvent('empresa_yakuza:hasExitedMarker', LastStation_yakuza, LastPart_yakuza, LastPartNum_yakuza)
					hasExited = true
				end

				HasAlreadyEnteredMarker_yakuza = true
				LastStation_yakuza             = currentStation
				LastPart_yakuza                = currentPart
				LastPartNum_yakuza             = currentPartNum

				TriggerEvent('empresa_yakuza:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end

			if not hasExited and not isInMarker and HasAlreadyEnteredMarker_yakuza then
				sleep = 1
				HasAlreadyEnteredMarker_yakuza = false
				TriggerEvent('empresa_yakuza:hasExitedMarker', LastStation_yakuza, LastPart_yakuza, LastPartNum_yakuza)
			end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'yakuza' and not isDead_yakuza then
			local playerPed      = PlayerPedId()
			local carro = IsPedInAnyVehicle(playerPed,  false)
			
			if CurrentAction_yakuza ~= nil then

				if IsControlJustReleased(0, Keys['E']) then

					if CurrentAction_yakuza == 'menu_vestiario' then
					
						MenuRoupa()
						
					elseif CurrentAction_yakuza == 'menu_cofre' then

						local other = {}
						other.maxweight = 999999999 -- Custom weight statsh.
						other.slots = 200 -- Custom slots spaces.
						TriggerServerEvent("inventory:server:OpenInventory", "stash", "yakuza", other)
						TriggerEvent("inventory:client:SetCurrentStash", "yakuza")
						
					elseif CurrentAction_yakuza == 'retirar_veiculo' then
						...
						if carro then
							...
							Garagem_Yakuza(CurrentActionData_yakuza.station, CurrentActionData_yakuza.partNum)
						end
						
					elseif CurrentAction_yakuza == 'guardar_veiculo' then
					
						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData_yakuza.vehicle)
						...
						ESX.TriggerServerCallback('johnny_garagens:getCarrosEmpresa',function(valid)	
							if (valid) then
								...
								TaskLeaveVehicle(playerPed, CurrentActionData_yakuza.vehicle, 0)
								Citizen.Wait(2500)
								ESX.Game.DeleteVehicle(CurrentActionData_yakuza.vehicle)
					...
					elseif CurrentAction_yakuza == 'menu_gestao_empresa' then
			...
			if IsControlJustReleased(0, Keys['F6']) and not isDead_yakuza and PlayerData.job ~= nil and PlayerData.job.name == 'yakuza' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'yakuza_actions') then
				MenuF6_Yakuza()
			end
