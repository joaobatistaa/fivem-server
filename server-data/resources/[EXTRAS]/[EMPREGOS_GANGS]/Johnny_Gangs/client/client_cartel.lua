...
local HasAlreadyEnteredMarker_cartel = false
local LastStation_cartel     = nil
local LastPart                = nil
local LastPartNum_cartel     = nil
local CurrentAction_cartel           = nil
local CurrentActionMsg_cartel        = ''
local CurrentActionData_cartel       = {}
local isDead_cartel                  = false
...
function Garagem_Cartel(station, partNum)		
	local foundSpawnPoint, spawnPoint = GetAvailable_SpawnPoint_Cartel(station, partNum)
	if foundSpawnPoint then
		TriggerEvent("johnny_garagens:carroempresa", spawnPoint, 'cartel')
	end 
end

function GetAvailable_SpawnPoint_Cartel(station, partNum)
	local spawnPoints = Config.cartelStations[station].Veiculos[partNum].SpawnPoints
...
AddEventHandler('empresa_cartel:hasEnteredMarker', function(station, part, partNum)

	if part == 'Vestiarios' then
	
		CurrentAction_cartel     = 'menu_vestiario'
		
	elseif part == 'Cofres' then
	
		CurrentAction_cartel     = 'menu_cofre'
		CurrentActionData_cartel = {station = station}
		
	elseif part == 'VeiculosSpawner' then

		CurrentAction_cartel     = 'retirar_veiculo'
		CurrentActionData_cartel = {station = station, partNum = partNum}
	...
	elseif part == 'GuardarVeiculos' then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			if DoesEntityExist(vehicle) then
				CurrentAction_cartel     = 'guardar_veiculo'
				CurrentActionData_cartel = {vehicle = vehicle}
			end
		end

	elseif part == 'GestaoEmpresa' then
	
		CurrentAction_cartel     = 'menu_gestao_empresa'
		
	end

end)

AddEventHandler('empresa_cartel:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction_cartel = nil
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'cartel' then
			...
			if isInMarker or (isInMarker and (LastStation_cartel ~= currentStation or LastPart_cartel ~= currentPart or LastPartNum_cartel ~= currentPartNum)) then
				sleep = 1
				if
					(LastStation_cartel ~= nil and LastPart_cartel ~= nil and LastPartNum_cartel ~= nil) and
					(LastStation_cartel ~= currentStation or LastPart_cartel ~= currentPart or LastPartNum_cartel ~= currentPartNum)
				then
					TriggerEvent('empresa_cartel:hasExitedMarker', LastStation_cartel, LastPart_cartel, LastPartNum_cartel)
					hasExited = true
				end

				HasAlreadyEnteredMarker_cartel = true
				LastStation_cartel             = currentStation
				LastPart_cartel                = currentPart
				LastPartNum_cartel             = currentPartNum

				TriggerEvent('empresa_cartel:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end

			if not hasExited and not isInMarker and HasAlreadyEnteredMarker_cartel then
				sleep = 1
				HasAlreadyEnteredMarker_cartel = false
				TriggerEvent('empresa_cartel:hasExitedMarker', LastStation_cartel, LastPart_cartel, LastPartNum_cartel)
			end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'cartel' and not isDead_cartel then
			local playerPed      = PlayerPedId()
			local carro = IsPedInAnyVehicle(playerPed,  false)
			
			if CurrentAction_cartel ~= nil then

				if IsControlJustReleased(0, Keys['E']) then

					if CurrentAction_cartel == 'menu_vestiario' then
					
						MenuRoupa()
						
					elseif CurrentAction_cartel == 'menu_cofre' then
					
						local other = {}
						other.maxweight = 999999999 -- Custom weight statsh.
						other.slots = 200 -- Custom slots spaces.
						TriggerServerEvent("inventory:server:OpenInventory", "stash", "cartel", other)
						TriggerEvent("inventory:client:SetCurrentStash", "cartel")
						
					elseif CurrentAction_cartel == 'retirar_veiculo' then
						...
						if carro then
							...
							Garagem_Cartel(CurrentActionData_cartel.station, CurrentActionData_cartel.partNum)
						end
						
					elseif CurrentAction_cartel == 'guardar_veiculo' then
			
						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData_cartel.vehicle)
						...
						ESX.TriggerServerCallback('johnny_garagens:getCarrosEmpresa',function(valid)	
							if (valid) then
								...
								TaskLeaveVehicle(playerPed, CurrentActionData_cartel.vehicle, 0)
								Citizen.Wait(2500)
								ESX.Game.DeleteVehicle(CurrentActionData_cartel.vehicle)
					...
					elseif CurrentAction_cartel == 'menu_gestao_empresa' then
			...
			if IsControlJustReleased(0, Keys['F6']) and not isDead_cartel and PlayerData.job ~= nil and PlayerData.job.name == 'cartel' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'cartel_actions') then
				MenuF6_Cartel()
			end
