...
local HasAlreadyEnteredMarker_medusa = false
local LastStation_medusa     = nil
local LastPart                = nil
local LastPartNum_medusa     = nil
local CurrentAction_medusa           = nil
local CurrentActionMsg_medusa        = ''
local CurrentActionData_medusa       = {}
local isDead_medusa                  = false
...
function Garagem_Medusa(station, partNum)		
	local foundSpawnPoint, spawnPoint = GetAvailable_SpawnPoint_Medusa(station, partNum)
	if foundSpawnPoint then
		TriggerEvent("johnny_garagens:carroempresa", spawnPoint, 'medusa')
	end 
end

function GetAvailable_SpawnPoint_Medusa(station, partNum)
	local spawnPoints = Config.medusaStations[station].Veiculos[partNum].SpawnPoints
...
function MenuF6_Medusa()
	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'medusa_actions',
	{
		title    = '🐍 Menu Medusa',
		align    = 'top-left',
		elements = {
...
AddEventHandler('empresa_medusa:hasEnteredMarker', function(station, part, partNum)

	if part == 'Vestiarios' then
	
		CurrentAction_medusa     = 'menu_vestiario'
		
	elseif part == 'Cofres' then
	
		CurrentAction_medusa     = 'menu_cofre'
		CurrentActionData_medusa = {station = station}
		
	elseif part == 'VeiculosSpawner' then

		CurrentAction_medusa     = 'retirar_veiculo'
		CurrentActionData_medusa = {station = station, partNum = partNum}
	...
	elseif part == 'GuardarVeiculos' then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			if DoesEntityExist(vehicle) then
				CurrentAction_medusa     = 'guardar_veiculo'
				CurrentActionData_medusa = {vehicle = vehicle}
			end
		end

	elseif part == 'GestaoEmpresa' then
	
		CurrentAction_medusa     = 'menu_gestao_empresa'
		
	end

end)

AddEventHandler('empresa_medusa:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction_medusa = nil
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'medusa' then
			...
			for k,v in pairs(Config.medusaStations) do
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'medusa' then
			...
			if isInMarker or (isInMarker and (LastStation_medusa ~= currentStation or LastPart_medusa ~= currentPart or LastPartNum_medusa ~= currentPartNum)) then
				sleep = 1
				if
					(LastStation_medusa ~= nil and LastPart_medusa ~= nil and LastPartNum_medusa ~= nil) and
					(LastStation_medusa ~= currentStation or LastPart_medusa ~= currentPart or LastPartNum_medusa ~= currentPartNum)
				then
					TriggerEvent('empresa_medusa:hasExitedMarker', LastStation_medusa, LastPart_medusa, LastPartNum_medusa)
					hasExited = true
				end

				HasAlreadyEnteredMarker_medusa = true
				LastStation_medusa             = currentStation
				LastPart_medusa                = currentPart
				LastPartNum_medusa             = currentPartNum

				TriggerEvent('empresa_medusa:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end

			if not hasExited and not isInMarker and HasAlreadyEnteredMarker_medusa then
				sleep = 1
				HasAlreadyEnteredMarker_medusa = false
				TriggerEvent('empresa_medusa:hasExitedMarker', LastStation_medusa, LastPart_medusa, LastPartNum_medusa)
			end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'medusa' and not isDead_medusa then
			local playerPed      = PlayerPedId()
			local carro = IsPedInAnyVehicle(playerPed,  false)
			
			if CurrentAction_medusa ~= nil then

				if IsControlJustReleased(0, Keys['E']) then

					if CurrentAction_medusa == 'menu_vestiario' then
					
						MenuRoupa()
						
					elseif CurrentAction_medusa == 'menu_cofre' then
					
						local other = {}
						other.maxweight = 999999999 -- Custom weight statsh.
						other.slots = 200 -- Custom slots spaces.
						TriggerServerEvent("inventory:server:OpenInventory", "stash", "medusa", other)
						TriggerEvent("inventory:client:SetCurrentStash", "medusa")
						
					elseif CurrentAction_medusa == 'retirar_veiculo' then
						...
						if carro then
							...
							Garagem_Medusa(CurrentActionData_medusa.station, CurrentActionData_medusa.partNum)
						end
						
					elseif CurrentAction_medusa == 'guardar_veiculo' then
						
						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData_medusa.vehicle)
						...
						ESX.TriggerServerCallback('johnny_garagens:getCarrosEmpresa',function(valid)	
							if (valid) then
								...
								TaskLeaveVehicle(playerPed, CurrentActionData_medusa.vehicle, 0)
								Citizen.Wait(2500)
								ESX.Game.DeleteVehicle(CurrentActionData_medusa.vehicle)
					...
					elseif CurrentAction_medusa == 'menu_gestao_empresa' then
			...
			if IsControlJustReleased(0, Keys['F6']) and not isDead_medusa and PlayerData.job ~= nil and PlayerData.job.name == 'medusa' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'medusa_actions') then
				MenuF6_Medusa()
			end
