...
local HasAlreadyEnteredMarker_ballas = false
local LastStation_ballas     = nil
local LastPart                = nil
local LastPartNum_ballas     = nil
local CurrentAction_ballas           = nil
local CurrentActionMsg_ballas        = ''
local CurrentActionData_ballas       = {}
local isDead_ballas                  = false
...
function Garagem_Ballas(station, partNum)		
	local foundSpawnPoint, spawnPoint = GetAvailable_SpawnPoint_Ballas(station, partNum)
	if foundSpawnPoint then
		TriggerEvent("johnny_garagens:carroempresa", spawnPoint, 'ballas')
	end 
end

function GetAvailable_SpawnPoint_Ballas(station, partNum)
	local spawnPoints = Config.ballasStations[station].Veiculos[partNum].SpawnPoints
...
AddEventHandler('empresa_ballas:hasEnteredMarker', function(station, part, partNum)

	if part == 'Vestiarios' then
	
		CurrentAction_ballas     = 'menu_vestiario'
		
	elseif part == 'Cofres' then
	
		CurrentAction_ballas     = 'menu_cofre'
		CurrentActionData_ballas = {station = station}
		
	elseif part == 'VeiculosSpawner' then

		CurrentAction_ballas     = 'retirar_veiculo'
		CurrentActionData_ballas = {station = station, partNum = partNum}
	...
	elseif part == 'GuardarVeiculos' then
		...
		if IsPedInAnyVehicle(playerPed,  false) then
			...
			if DoesEntityExist(vehicle) then
				CurrentAction_ballas     = 'guardar_veiculo'
				CurrentActionData_ballas = {vehicle = vehicle}
			end
		end

	elseif part == 'GestaoEmpresa' then
	
		CurrentAction_ballas     = 'menu_gestao_empresa'
		
	end

end)

AddEventHandler('empresa_ballas:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction_ballas = nil
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'ballas' then
			...
			if isInMarker or (isInMarker and (LastStation_ballas ~= currentStation or LastPart_ballas ~= currentPart or LastPartNum_ballas ~= currentPartNum)) then
				sleep = 1
				if
					(LastStation_ballas ~= nil and LastPart_ballas ~= nil and LastPartNum_ballas ~= nil) and
					(LastStation_ballas ~= currentStation or LastPart_ballas ~= currentPart or LastPartNum_ballas ~= currentPartNum)
				then
					TriggerEvent('empresa_ballas:hasExitedMarker', LastStation_ballas, LastPart_ballas, LastPartNum_ballas)
					hasExited = true
				end

				HasAlreadyEnteredMarker_ballas = true
				LastStation_ballas             = currentStation
				LastPart_ballas                = currentPart
				LastPartNum_ballas             = currentPartNum

				TriggerEvent('empresa_ballas:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end

			if not hasExited and not isInMarker and HasAlreadyEnteredMarker_ballas then
				sleep = 1
				HasAlreadyEnteredMarker_ballas = false
				TriggerEvent('empresa_ballas:hasExitedMarker', LastStation_ballas, LastPart_ballas, LastPartNum_ballas)
			end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'ballas' and not isDead_ballas then
			local playerPed      = PlayerPedId()
			local carro = IsPedInAnyVehicle(playerPed,  false)
			
			if CurrentAction_ballas ~= nil then

				if IsControlJustReleased(0, Keys['E']) then

					if CurrentAction_ballas == 'menu_vestiario' then
					
						MenuRoupa()
						
					elseif CurrentAction_ballas == 'menu_cofre' then
					
						local other = {}
						other.maxweight = 999999999 -- Custom weight statsh.
						other.slots = 200 -- Custom slots spaces.
						TriggerServerEvent("inventory:server:OpenInventory", "stash", "ballas", other)
						TriggerEvent("inventory:client:SetCurrentStash", "ballas")
						
					elseif CurrentAction_ballas == 'retirar_veiculo' then
						...
						if carro then
							...
							Garagem_Ballas(CurrentActionData_ballas.station, CurrentActionData_ballas.partNum)
						end
						
					elseif CurrentAction_ballas == 'guardar_veiculo' then
	
						local vehicleProps = ESX.Game.GetVehicleProperties(CurrentActionData_ballas.vehicle)
						...
						ESX.TriggerServerCallback('johnny_garagens:getCarrosEmpresa',function(valid)	
							if (valid) then
								...
								TaskLeaveVehicle(playerPed, CurrentActionData_ballas.vehicle, 0)
								Citizen.Wait(2500)
								ESX.Game.DeleteVehicle(CurrentActionData_ballas.vehicle)
					...
					elseif CurrentAction_ballas == 'menu_gestao_empresa' then
			...
			if IsControlJustReleased(0, Keys['F6']) and not isDead_ballas and PlayerData.job ~= nil and PlayerData.job.name == 'ballas' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'ballas_actions') then
				MenuF6_Ballas()
			end
