...
RegisterNetEvent("mumble:falar")
AddEventHandler("mumble:falar", function(radio, name)	
	...
	for id, players in pairs(radioData) do
		for player, _ in pairs(players) do
			...
			--print(player)
			if id == radio then
				TriggerClientEvent('mythic_notify:client:SendAlert', player, { type = 'azul', text =  'A transmitir: ' ..name} )
			end
		end
	end
end)
...
AddEventHandler("mumble:SetVoiceData", function(key, value, target)
	...
	if key == "radio" and radioChannel ~= value then -- Check if channel has changed
		if radioChannel > 0 then -- Check if player was in a radio channel
			if radioData[radioChannel] then  -- Remove player from radio channel
				if radioData[radioChannel][source] then
                    ...
                    TriggerClientEvent('mumble:saidaradio', -1, name, radioChannel)
		...
		if value > 0 then
			...
			local name = GetPlayerName(source)
			TriggerClientEvent('mumble:entradaradio', -1, name, value)
