...
local notificacoes = true
local entradas = true
local micClickVolume = mumbleConfig.micClickVolume
...
RegisterCommand('restartvoz', function(source, args)
		...
		if value > 0 then
			...
			if playerServerId == player then
				if mumbleConfig.showRadioList then
					...
					SendNUIMessage({ radioId = playerServerId, radioName = GetPlayerName(PlayerId()), self = true }) -- Add client to radio list
				end
...
RegisterNetEvent("mumble:entradaradio")
AddEventHandler("mumble:entradaradio", function(name, value)
 	--local player = source 
--	local radioChannel = voiceData[1]["radio"] or 0
	if entradas then
		local playerData = voiceData[playerServerId]
		
		if playerData ~= nil then
			--print(playerData.radio)
			if playerData.radio == value then
				exports['mythic_notify']:DoHudText('success', '' ..name.. ' Entrou na rádio')
			end
		end
	end
end)

RegisterNetEvent("mumble:saidaradio")
AddEventHandler("mumble:saidaradio", function(name, radioChannel)
 	--local player = source 
--	local radioChannel = voiceData[1]["radio"] or 0
	if entradas then
		local playerData = voiceData[playerServerId]
		if playerData ~= nil then
			...
			if playerData.radio == radioChannel then
				exports['mythic_notify']:DoHudText('error', '' ..name.. ' Saiu  da rádio')
			end
		end
	end
end)
		...
		if initialised then
			...
			if IsControlJustPressed(0, mumbleConfig.controls.proximity.key) then
				...
				if not secondaryPressed then
					...
					TriggerEvent("mumble-voip:VoiceRangeChanged", mumbleConfig.voiceModes[voiceMode][1])
				...
				if not mumbleConfig.controls.radio.pressed then
					if IsControlJustPressed(0, mumbleConfig.controls.radio.key) then
						if playerData.radio > 0 then
							TriggerEvent("mumble-voip:RadioTrafficStateChanged", true)
							...
							if notificacoes then
								...
								local radio = playerData.radio
								TriggerServerEvent('mumble:falar',  radio, name)
							end
							...
							Citizen.CreateThread(function()
								...
								TriggerEvent("mumble-voip:RadioTrafficStateChanged", false) 
...
RegisterCommand('radio', function(source, args)
	...
	if args[1]~=nil then --(args[1]=='notificacoes' or args[1]=='som' or args[1]=='entradas') then
		
		if args[1] == 'notificacoes' then
			notificacoes = not notificacoes
			if notificacoes then
				exports['mythic_notify']:DoHudText('roxo', 'Notificações da rádio ativadas')
			else
				exports['mythic_notify']:DoHudText('roxo', 'Notificações da rádio desativadas')
			end
		elseif args[1] == 'som' then
			if args[2]~= nil then
				if tonumber(args[2])>=0 and tonumber(args[2])<=1 then
					micClickVolume = tonumber(args[2])
					local perc = (micClickVolume/1)*100
					exports['mythic_notify']:DoHudText('roxo', 'Som dos alertas da rádio definido para '..perc..'%')
				end
			end
		elseif args[1] == 'volume' then
			if args[2]~= nil then
				if tonumber(args[2])>=0 and tonumber(args[2])<=1 then
					radioVolume = tonumber(args[2])
					local playerServerId = GetPlayerServerId(PlayerId())
					MumbleSetVolumeOverrideByServerId(playerServerId, radioVolume)
					local perc = (radioVolume/1)*100
					exports['mythic_notify']:DoHudText('roxo', 'Volume da rádio definido para '..perc..'%')
				end
			end
				
		else
			if args[1] == 'entradas' then
				entradas = not entradas
				if entradas then
					exports['mythic_notify']:DoHudText('roxo', 'Entradas/Saídas da rádio ativadas')
				else
					exports['mythic_notify']:DoHudText('roxo', 'Entradas/Saídas da rádio desativadas')
				end
			end
		end
		
	else
		exports['mythic_notify']:DoHudText('inform', '/radio (entradas/som [0-1]/volume [0-1]/notificacoes')
	end
end)
...
Citizen.CreateThread(function()
	while true do
		if initialised then
			...
			if playerRadio==0 or playerRadio==nil then
				nomeradio = 'Nenhuma'
			elseif playerRadio>0 and playerRadio <=9 then
				nomeradio = 'Forças de Socorro'
			elseif playerRadio>9 and playerRadio <=39 then
				nomeradio = 'PSP'
			elseif playerRadio>39 and playerRadio <=59 then
				nomeradio = 'INEM'
			elseif playerRadio>59 and playerRadio <=69 then
				nomeradio = 'Norauto'
			elseif playerRadio>69 and playerRadio <=79 then
				nomeradio = 'Ministério Público'
			elseif playerRadio>79 and playerRadio <=89 then
				nomeradio = 'Vagos'
			elseif playerRadio>89 and playerRadio <=99 then
				nomeradio = 'Grove'
			elseif playerRadio>99 and playerRadio <=109 then
				nomeradio = 'Ballas'
			elseif playerRadio>109 and playerRadio <=119 then
				nomeradio = 'Vanilla'
			elseif playerRadio>119 and playerRadio <=129 then
				nomeradio = 'Bahamas'
			elseif playerRadio>129 and playerRadio <=139 then
				nomeradio = 'Máfia'
			elseif playerRadio>139 and playerRadio <=149 then
				nomeradio = 'PeakyBlinders'
			elseif playerRadio>149 and playerRadio <=159 then
				nomeradio = 'N/A'
			elseif playerRadio>159 and playerRadio <=169 then
				nomeradio = 'N/A'
			elseif playerRadio>169 and playerRadio <=179 then
				nomeradio = 'N/A'
			elseif playerRadio>179 and playerRadio <=189 then
				nomeradio = 'N/A'
			elseif playerRadio>189 and playerRadio <=199 then
				nomeradio = 'N/A'
			elseif playerRadio>199 and playerRadio <=209 then
				nomeradio = 'N/A'
			elseif playerRadio>209 and playerRadio <=219 then
				nomeradio = 'N/A'
			elseif playerRadio>219 and playerRadio <=229 then
				nomeradio = 'N/A'
			elseif playerRadio>229 and playerRadio <=239 then
				nomeradio = 'N/A'
			elseif playerRadio>239 and playerRadio <=249 then
				nomeradio = 'Staff'
			else
				nomeradio = 'Público'
			end
			...
			SendNUIMessage({
				...
				radio = teste, --mumbleConfig.radioChannelNames[playerRadio] ~= nil and mumbleConfig.radioChannelNames[playerRadio] or playerRadio,
				--radio = nomeradio,
