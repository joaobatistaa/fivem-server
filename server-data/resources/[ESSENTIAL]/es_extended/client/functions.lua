...
function ESX.UI.Menu.Open(type, namespace, name, data, submit, cancel, change, close)
    ...
    menu.setElement = function(i, key, val)
		if i ~= nil and key ~= nil and val ~= nil then
...
function ESX.ShowNotification(message, type, length)
    if GetResourceState("Johnny_Notificacoes") ~= "missing" then
		if type == nil then
			type = 'inform'
		end
        return SendTextMessage(message, type)
    end

    print("[^1ERROR^7] ^5Johnny Notificacoes^7 is Missing!")
end
