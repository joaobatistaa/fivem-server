...
local newPlayer = 'INSERT INTO `users` SET `name` = ?, `accounts` = ?, `identifier` = ?, `license` = ?, `group` = ?'
local loadPlayer = 'SELECT `accounts`, `job`, `job_grade`, `group`, `position`, `inventory`, `skin`, `loadout`, `vip`'
...
function createESXPlayer(identifier, playerId, data)
	...
	local license = ESX.GetLicense(playerId)
	local nomeSteam = GetPlayerName(playerId)
	
	for account, money in pairs(Config.StartingAccountMoney) do
		accounts[account] = money
	end
	
	local group = Core.IsPlayerAdmin(playerId)
	if group then
	...
	if not Config.Multichar then
		MySQL.prepare(newPlayer, {nomeSteam, json.encode(accounts), identifier, license, defaultGroup}, function()
			loadESXPlayer(identifier, license, playerId, true)
		end)
	else
		MySQL.prepare(newPlayer,
		{nomeSteam, json.encode(accounts), identifier, defaultGroup, data.firstname, data.lastname, data.dateofbirth, data.sex, data.height}, function()
			loadESXPlayer(identifier, license, playerId, true)
		end)
	end
end
...
function loadESXPlayer(identifier, license, playerId, isNew)
  ...
  -- Identity
    ...
    userData.loadout, userData.playerName, userData.coords, userData.vip)
...
AddEventHandler('chatMessage', function(playerId, author, message)
	...
	if message:sub(1, 1) == '/' and playerId > 0 then
		...
		TriggerClientEvent('chat:addMessage', playerId, {
			template = '<div class="chat-message system"><i class="fas fa-cog"></i> <b><span style="color: #df7b00">WTRP SYSTEM</span>&nbsp;|&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{1}</span></b><div style="margin-top: 5px; font-weight: 300;"><b>{0}</b> não é um comando válido!</div></div>',
