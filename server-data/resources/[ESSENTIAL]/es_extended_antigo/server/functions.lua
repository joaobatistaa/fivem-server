        ...
        if playerId == 0 then
          print(('[^3WARNING^7] %s^7'):format(error))
			...
			TriggerClientEvent('chat:addMessage', xPlayer.source, {
				template = '<div class="chat-message error"><i class="fas fa-ban"></i> <b><span style="color: #f81111">WTRP SYSTEM</span>&nbsp;|&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{0}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
				args = { time, error }
			})
        end
      else
...
function ESX.GetLicense(playerId)
	local fxDk = GetConvarInt('sv_fxdkMode', 0) 
	if fxDk == 1 then
		return "ESX-DEBUG-LICENCE"
	end
...
function ESX.DiscordLog(name, title, color, message)
  ...
  local embedData = {{
      ...
      ['footer'] = {
          ['text'] = "| Johnny Logs | " .. os.date(),
          ['icon_url'] = "https://i.ibb.co/bFvY8vJ/worldtuga.gif"
      },
      ['description'] = message,
      ['author'] = {
          ['name'] = "WorldTuga RP",
